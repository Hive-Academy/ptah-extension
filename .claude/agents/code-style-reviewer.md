...
- Clarification trigger: no batch, diff or file list, or the task deliberately contradicts the repository's stated rules with no document saying why; stop before code-style-review.md and any verdict. Proceed when files are named; a thin spec is a finding, not a question.
...
- Never manufacture a finding. When none holds, state the scope examined, the checks performed and the remaining uncertainty, so a clean verdict on the structure is auditable.
...
5. `ptah_get_diagnostics` and any applicable static-analysis or verification output
   available for the affected code. If you run a check yourself, scope it with `-p <project>` to the
