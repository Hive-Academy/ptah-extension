...
## Batch 3: Extended RPC Methods (Config, Command, Wizard, Quality, Agent) [COMPLETE]

**Developer**: backend-developer
**Tasks**: 6 | **Dependencies**: Batch 1 (DI shim must be in place for workspace-intelligence services)
**Commit**: 50e90ab6

### Task 3.1: Add config extended methods (4 methods) [COMPLETE]
...
### Task 3.2: Add command:execute and agent:stop methods [COMPLETE]
...
### Task 3.3: Add quality:export method [COMPLETE]
...
### Task 3.4: Add wizard methods -- setup-wizard:launch, wizard:cancel, wizard:cancel-analysis (3 methods) [COMPLETE]
...
### Task 3.5: Add wizard methods -- deep-analyze, list-analyses, load-analysis, recommend-agents (4 methods) [COMPLETE]
...
### Task 3.6: Add wizard methods -- submit-selection, retry-item (2 methods) [COMPLETE]
...
- Change the inline type annotation from `message: string` to `prompt: string` in the params type
- Update the full inline type to match `ChatContinueParams` fields: `{ sessionId: string; prompt: string; tabId: string; workspacePath?: string; model?: string; files?: string[]; images?: unknown[] }`
- Note: `tabId` is REQUIRED per the ChatContinueParams contract, not optional
- Change validation from `!params?.message` to `!params?.prompt`
- Change error message from `'sessionId and message are required'` to `'sessionId and prompt are required'`
- Change `sdkAdapter.sendMessageToSession(params.sessionId, params.message, ...)` to use `params.prompt`
- Change `files: params.contextFiles` to `files: params.files` (matching ChatContinueParams field name)

**Implementation Details**:

- The `ChatContinueParams` interface (rpc.types.ts:91-115) uses `prompt` not `message`, `files` not `contextFiles`
- The frontend sends `prompt` -- using `message` means the value is always undefined
- This is the most critical bug: chat:continue is completely broken due to wrong field name
...
- In the catch block of `streamEventsToRenderer()` (around line 220-225), after logging the error, resolve webviewManager and broadcast `MESSAGE_TYPES.CHAT_COMPLETE`
- The catch block must handle the case where webviewManager itself fails to resolve (nested try/catch)
- Use the same `routingId` and `sessionId` pattern as the happy path

**Implementation Details**:

- Current catch block only does `console.error` (which itself is Issue #7 -- will be fixed in Task 4.6)
- Add in the catch block:
  ...
  ```
- This ensures the frontend always transitions out of the loading/streaming state
...
- Update params type from `{ sessionId: string }` to `{ sessionId: string; tabId: string; workspacePath?: string; model?: string }` (matching `ChatResumeParams` from rpc.types.ts)
- Use `params.workspacePath` with fallback: `params.workspacePath || workspaceProvider.getWorkspaceRoot() || ''`
- After reading session history, call `historyReader.readHistoryAsMessages(sessionId, workspacePath)` to get backward-compatible messages
- After reading history, call `subagentRegistry.registerFromHistoryEvents(events, sessionId)` to populate the registry with interrupted agents from history
- Return the `messages` array from `readHistoryAsMessages()` instead of hardcoded `messages: []`

**Implementation Details**:

- VS Code handler at chat-rpc.handlers.ts:1130-1184 shows the correct pattern:
  1. Read events+stats via `historyReader.readSessionHistory()`
  2. Read messages via `historyReader.readHistoryAsMessages()`
  3. Call `subagentRegistry.registerFromHistoryEvents(events, sessionId)`
  4. Query `subagentRegistry.getResumableBySession(sessionId)` (already done)
- Add `readHistoryAsMessages` to the inline type for `SDK_SESSION_HISTORY_READER`:
  ```typescript
  readHistoryAsMessages(sessionId: string, workspacePath: string): Promise<unknown[]>;
  ```
- Add `registerFromHistoryEvents` to the inline type for `SUBAGENT_REGISTRY_SERVICE`:
  ```typescript
  registerFromHistoryEvents(events: unknown[], sessionId: string): number;
  ```
- Use `params.workspacePath || workspaceProvider.getWorkspaceRoot() || ''` for workspace resolution
...
- Before calling `sdkAdapter.sendMessageToSession()`, query `subagentRegistry.getResumableBySession(sessionId)`
- If resumable subagents exist, build a context prefix and prepend it to the prompt
- After injection, remove injected subagents from registry (one-shot injection)
- Simplified version for Electron: skip transcript file existence check (no VS Code filesystem watcher setup)

**Implementation Details**:

- Resolve `TOKENS.SUBAGENT_REGISTRY_SERVICE` in the chat:continue handler (with try/catch)
- Query `getResumableBySession(params.sessionId)` for interrupted agents
- If agents found, build context prefix (pattern from VS Code chat-rpc.handlers.ts:928-957):
  ```
  [SYSTEM CONTEXT - INTERRUPTED AGENTS]
  The following subagent(s) were interrupted and did not complete their work:
    - {agentType} agent (agentId: {agentId}) - interrupted {N} min ago
  IMPORTANT INSTRUCTIONS:
  1. Your FIRST action should be to resume these interrupted agents using the Task tool with the "resume" parameter set to the agentId shown above.
  ...
  4. If the user explicitly asks to start fresh or work on something completely unrelated, you may skip resumption.
  [END SYSTEM CONTEXT]
  ```
- Prepend to prompt: `enhancedPrompt = contextPrefix + params.prompt`
- After injection, remove agents from registry: `subagentRegistry.remove(s.toolCallId)` for each
- Use try/catch around the entire subagent block -- failure must not break chat:continue
- Use the `getResumableBySession` method (type: `Array<{ agentId: string; agentType: string; toolCallId: string; interruptedAt?: number }>`)
...
**NOTE**: This task is largely covered by Task 4.1. The remaining work is ensuring the `workspacePath` field is accepted and used.

**Quality Requirements**:

- Ensure `workspacePath` is in the params type and used for workspace resolution (fallback to IWorkspaceProvider)
- Use `params.workspacePath || workspaceProvider.getWorkspaceRoot() || ''` for the auto-resume config `projectPath`
- This allows the frontend to pass explicit workspace paths for multi-workspace scenarios
...
- Resolve `TOKENS.LOGGER` from container at the top of `setupRpcHandlers()` and pass as parameter to all register functions and helpers
- Replace ALL `console.error(...)` calls with `logger.error(...)`
- There are 3 instances of console.error:
  1. `streamEventsToRenderer` catch block (line 221)
  2. `chat:start` streaming error `.catch` callback (line 305)
  3. `chat:resume` error catch block (line 481)
- Use `logger.error(message, error instanceof Error ? error : new Error(String(error)))` pattern

**Implementation Details**:

- Resolve once in `setupRpcHandlers()`: `const logger = container.resolve<Logger>(TOKENS.LOGGER);`
- Update function signatures to accept logger:
  - `registerChatMethods(container, rpcHandler, logger)`
  - `streamEventsToRenderer(container, sessionId, stream, tabId, logger)` (add logger param)
- Import `Logger` type from `@ptah-extension/vscode-core` (already imported as type from RpcHandler line)
- Update all callers to pass logger
...
- Change `chat:abort` handler to use `interruptSession` instead of `abortSession`
- Update the inline SDK adapter type from `abortSession(sessionId: string)` to `interruptSession(sessionId: string)`
- This standardizes both abort paths to use the correct SDK method name

**Implementation Details**:

- Current code at lines 328-331: resolve sdkAdapter with `abortSession`, call `sdkAdapter.abortSession(params.sessionId)`
- Change to: resolve sdkAdapter with `interruptSession`, call `sdkAdapter.interruptSession(params.sessionId)`
...
- Extract to a module-level constant: `const DEFAULT_MODEL = 'claude-sonnet-4-20250514';`
- Place near the top of the file after imports, before function definitions
- Replace ALL occurrences in rpc-handler-setup.ts (3 instances in chat:start and chat:continue)
- Use `storageService.get('model.selected', DEFAULT_MODEL)` pattern everywhere
- Export the constant so rpc-method-registration.service.ts can import it (or define separately in both)

**Implementation Details**:

- Search for all `'claude-sonnet-4-20250514'` occurrences and replace with `DEFAULT_MODEL`
- Add `export` if sharing: `export const DEFAULT_MODEL = 'claude-sonnet-4-20250514';`

---

**Batch 4 Verification**:

- chat:continue uses `params.prompt` (not `params.message`) and `params.files` (not `params.contextFiles`)
- streamEventsToRenderer sends CHAT_COMPLETE on error
- chat:resume accepts tabId/workspacePath, calls registerFromHistoryEvents and readHistoryAsMessages
- chat:continue injects subagent context before sending message
- All console.error replaced with logger.error
- chat:abort uses interruptSession
- DEFAULT_MODEL constant used everywhere
- Build passes: `npx nx build ptah-electron`
- code-logic-reviewer approved
...
- Change the inline type annotation from `{ enabledPlugins: string[] }` to `{ enabledPluginIds: string[] }`
- Change `params.enabledPlugins` to `params.enabledPluginIds` in the storage set call
- Update the validation error message from `'enabledPlugins is required'` to `'enabledPluginIds is required'`
- Storage key remains `'plugins.enabled'` -- only the param field name changes

**Implementation Details**:

- Current code at lines 553-572 uses `params.enabledPlugins`
- The frontend sends `enabledPluginIds` per the RPC contract
- This means plugin config saves silently fail (storing undefined)
...
- Add a module-level `let generationStartedAt: number | null = null;` alongside the existing `isGenerating` flag (line 43)
- Define a staleness threshold constant: `const GENERATION_STALENESS_MS = 5 * 60 * 1000;` (5 minutes)
- Wherever `isGenerating` is checked before returning early (in wizard:submit-selection and wizard:retry-item), add staleness check first:
  ```typescript
  if (isGenerating && generationStartedAt && Date.now() - generationStartedAt > GENERATION_STALENESS_MS) {
    logger.warn('[Electron RPC] Generation guard was stale, resetting');
    isGenerating = false;
    generationStartedAt = null;
  }
  ```
- Set `generationStartedAt = Date.now()` whenever `isGenerating = true` is set
- Reset `generationStartedAt = null` whenever `isGenerating = false` is set (in finally blocks and wizard:cancel)

**Implementation Details**:

- The `isGenerating` flag is at line 43
- Add `generationStartedAt` right after it on line 44
- Add `GENERATION_STALENESS_MS` constant before the flags
- Locations to update:
  - wizard:submit-selection: check before early return, set on start, reset in finally
  - wizard:retry-item: check before early return, set on start, reset in finally
  - wizard:cancel: reset both flags
...
- At the end of `registerConfigExtendedMethods()`, after the config:model-switch registration, add initialization code
- Read `autopilot.enabled` and `autopilot.permissionLevel` from storage
- If enabled and permissionLevel is not 'ask', call `permissionHandler.setPermissionLevel(permissionLevel)`
- Wrap in try/catch -- this is best-effort initialization, must not break startup

**Implementation Details**:

- Add at the end of `registerConfigExtendedMethods()` function body (after config:model-switch registration, before the closing `}`):
  ```typescript
  ...
  try {
    ...
    const savedLevel = initStorageService.get('autopilot.permissionLevel', 'ask');
    if (savedEnabled && savedLevel !== 'ask') {
      ...
      logger.info('[Electron RPC] Initialized permission handler from saved config', {
...
- Add a module-level constant: `const DEFAULT_MODEL = 'claude-sonnet-4-20250514';`
- Place near the `isGenerating` flag at the top of the file
- Replace ALL occurrences of the hardcoded string in this file
- If Task 4.8 exports the constant from rpc-handler-setup.ts, import it instead of defining locally

**Implementation Details**:

- Search for `'claude-sonnet-4-20250514'` in rpc-method-registration.service.ts
- Found in: config:models-list (line 910), wizard:deep-analyze (lines 1293 and 1296)
- Replace each with `DEFAULT_MODEL`

---

**Batch 5 Verification**:

- plugins:save-config uses `params.enabledPluginIds` (not `params.enabledPlugins`)
- isGenerating has staleness timeout (5 minutes)
- generationStartedAt set/reset at all isGenerating mutation points
- Permission handler initialized from saved config on startup
- DEFAULT_MODEL constant used everywhere
- Build passes: `npx nx build ptah-electron`
- code-logic-reviewer approved
