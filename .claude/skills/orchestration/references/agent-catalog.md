...
**Note**: The per-role examples below are the injection templates appended to sub-agent prompts. For extended examples with full `ptah_agent_spawn` calls and detailed task prompts, see [cli-agent-delegation.md](cli-agent-delegation.md#per-role-delegation-examples).
...
See [cli-agent-delegation.md](cli-agent-delegation.md) for the comprehensive delegation reference.
