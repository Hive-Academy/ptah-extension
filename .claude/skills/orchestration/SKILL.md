...
**Prefer the `tasks:create` RPC/CLI when available** — it does the atomic
allocate-reserve-write below in one call (`TaskWriterService.create`) and cannot
clobber a concurrent session. Hand-allocate only when no host is running, and
