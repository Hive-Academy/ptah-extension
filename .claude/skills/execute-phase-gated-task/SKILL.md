...
1. **Read all specification documents in order.** Locate and read: implementation plan (phase breakdown, file manifest, constraints), context/decisions, task description, and reference docs (design specs, content, examples). Note the phase/batch order and any dependency graph.
...
4. **Read all source files mentioned in the batch** before writing. Use absolute paths. For files you'll edit, scan for style (naming, error handling, comment density, async patterns) to match surrounding code. For reference files (specs, existing sections), extract the actual patterns and selectors.
...
6. **Validate after the batch.** Run all verification commands the plan specifies (e.g., `npx nx build <app>`, `npx nx lint <app>`, grep checks for guard patterns, visual inspection of rendered output or video frames). Document the results. If validation fails, diagnose and fix before proceeding.
...
- **Validation gates are hard**: Build/lint/test failures block progression. Do not skip; diagnose and fix the root cause, then re-run.
