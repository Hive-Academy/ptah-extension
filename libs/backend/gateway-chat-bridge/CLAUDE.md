...
- **Permission-prompt notices (TASK_2026_271 #1)**: at `'ask'` a gateway turn's `Write`/`Bash`/network/MCP prompt is unroutable (the `gw-*` tab id is not a UUID) — it goes to the local Ptah window with a 60 s deny window. The bridge subscribes to `SdkPermissionHandler.onPromptLifecycle` and, for `gw-*` routing hints of a running turn, sends an out-of-band notice via `gateway.sendNotice` ("Ptah needs approval to run `Write`… within 60s…") and, on `'timed-out'`, "No approval arrived… skipped it". Notices bypass the coalescer; the reply buffer is untouched. Routable prompts (real webview surface, no timeout) are ignored.
- **Typing indicator (TASK_2026_271)**: `runTurn` pings `gateway.sendTyping` as soon as the workspace resolves — before the SDK context is even built — and re-arms it every 8 s (`TYPING_REARM_MS`; Discord's indicator decays after ~10 s) until the turn's `finally` clears the interval. The interval is `unref()`'d and checks the turn's cancellation flag, so a watchdog-terminated turn stops typing immediately and nothing types after the reply lands. A turn that fails closed on workspace resolution never types at all.
- **`sendError` drains AND discards (TASK_2026_271, verification review)**: the coalescer's `body` is cumulative per conversation key and `drainOutbound` alone leaves it in place — only `discardOutbound` clears it. Every error reply that is not followed by `completeOutboundTurn` therefore stranded its own text in the buffer and got prepended to the NEXT turn's reply: the two fail-closed workspace exits (they return before the seal is even wired) and the post-seal delivery-failure reply. The discard lives in `sendError` itself, not at the call sites, so any future early return inherits the guarantee; the sites that do seal afterwards see a no-op discard. Regression specs run the real `StreamCoalescer` and assert on a SECOND turn's delivered body — asserting "discard was called" passes against a mock with no buffer to leak.
- **Delivery failure is reported (TASK_2026_271 #2)**: if the end-of-turn seal throws (`OutboundDeliveryError` from `flushOutbound`), the bridge sends `DELIVERY_FAILED_MESSAGE` on a fresh message instead of only warn-logging.
- **Every exit reports an outcome (TASK_2026_271 #7)**: `recordTurnOutcome` is what the Gateway tab reads, and the `finally` that calls it is wired only AFTER the two fail-closed workspace exits have already returned. Those two therefore call it themselves, as does the `resolveSdkContext` catch — so a turn that dies before the seal answers the user, marks the row `'failed'` AND shows red in the tab. Reporting two of those three is the partial silence this task exists to close; the regression specs assert the outcome, not just the reply.
...
- **Restart recovery — notify, never replay (TASK_2026_277)**: `runTurn` stamps the inbound row `'running'` at turn start and `'done'`/`'failed'` in the SAME `finally` that calls `recordTurnOutcome` (one turn lifecycle, not two), plus `'failed'` on the two fail-closed workspace exits and on an `resolveSdkContext` throw. `start()` then fires `notifyInterrupted()` — deliberately un-awaited, since host activation must not wait on a chat platform round-trip — which calls `GatewayService.claimInterruptedInboundTurns()` and sends ONE `sendNotice` per conversation: "Ptah restarted while working on your last message. Please send it again." The claim marks the rows `'interrupted'` before it returns, so a failed notice is dropped rather than retried forever; a claim that throws is warn-logged and inbound subscription still proceeds. **No agent turn is ever started for an interrupted row** — a turn that died at `auto-edit`/`yolo` may already have run `Write`/`Bash`, and replaying it would repeat those side effects with no idempotency guarantee. Opt-in replay is explicitly out of scope and would need a per-turn idempotency record first.
