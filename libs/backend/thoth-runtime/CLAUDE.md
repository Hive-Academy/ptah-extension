...
- **The memory retention job is one spec, two registrations** (TASK_2026_440). `MEMORY_RETENTION_JOB` (`@ptah/memory-retention`, handler `memory:retention`, `17 * * * *` UTC — a constant, off every other built-in slot) and `createMemoryRetentionHandler(container)` live in `memory-retention-job.ts`; `startThothCron` registers them behind `isRegistered(MEMORY_TOKENS.MEMORY_RETENTION_SERVICE)` + `handlerRegistry.has`, wrapped in `withActivityEmit`, and `cli-engine` registers the same pair without an emitter. The handler resolves the service (resolve failure → `skipped`, `retention-service-unavailable`), the power monitor and the optional `FOREGROUND_ACTIVITY_TRACKER` PER RUN (no tracker → `Infinity`). It maps `skipped` → skipped outcome, `completed`/`partial` → summary, and THROWS on `failed` with the reason token only, so the run row records `failed` — `MemoryRetentionService.run` itself never rejects. Registration runs no SQL beyond the upsert.
