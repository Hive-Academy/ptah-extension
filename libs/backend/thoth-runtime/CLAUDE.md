...
Owns: SQLite open/migrate + `sqlite-vec` diagnostics, memory curator + memory trigger, memory/vec/embedder push bridges, skill synthesis + skill trigger, code-symbol indexing run-deps, the workspace file index, and the cron scheduler with its built-in daily backup job, nightly database integrity-check job and hourly memory retention job (plus the shared retention job spec and handler factory the CLI host also registers).
