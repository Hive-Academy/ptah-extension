...
Owns: SQLite open/migrate + `sqlite-vec` diagnostics, memory curator + memory trigger, memory/vec/embedder push bridges, skill synthesis + skill trigger, code-symbol indexing run-deps, the workspace file index, and the cron scheduler with its built-in daily backup job, nightly database integrity-check job, hourly memory retention job and hourly one-time skills backlog cleanup job (plus the shared job specs and handler factories the CLI host also registers).
...
SKILL_DRAIN_JOBS / SkillDrainJobSpec
MEMORY_RETENTION_JOB / MemoryRetentionJobSpec
createMemoryRetentionHandler(container): JobHandler
SKILL_BACKLOG_CLEANUP_JOB / SkillBacklogCleanupJobSpec
createSkillBacklogCleanupHandler(container): JobHandler
...
- **The daily backup runs OUT OF PROCESS and takes no database handle** (TASK_2026_383). `@ptah/daily-backup` (`0 3 * * *` UTC, handler `backup:daily`) calls `backupSvc.backup('daily')` — one argument; the worker opens the file itself, read-only — then `rotate('daily', KEEP_BY_KIND.daily)` (never a literal). **The `refs.sqliteConnection` guard sits BELOW that call and gates only `pragma('optimize')`**, which is the only part of the job that still needs a live handle. The fixed 100-page post-backup incremental vacuum was deleted (TASK_2026_440): free pages are reclaimed only by the memory retention job, in bounded steps through `SqlitePageReclaimer`. Do not re-add a vacuum here. It used to sit above the backup, which silently skipped the daily backup on every host with no live connection — the host that most needs one. Do not move it back up, and do not re-add a `db` argument. A `null` return is not an error: the summary reads `backup not taken; see the database.backup degradation report`, and the reason is a `'critical'` degradation event from `SqliteBackupService`, not a throw here.
...
- **The memory retention job is one spec, two registrations** (TASK_2026_440). `MEMORY_RETENTION_JOB` (`@ptah/memory-retention`, handler `memory:retention`, `17 * * * *` UTC — a constant, off every other built-in slot) and `createMemoryRetentionHandler(container)` live in `memory-retention-job.ts`; `startThothCron` registers them behind `isRegistered(MEMORY_TOKENS.MEMORY_RETENTION_SERVICE)` + `handlerRegistry.has`, wrapped in `withActivityEmit`, and `cli-engine` registers the same pair without an emitter. The handler resolves the service (resolve failure → `skipped`, `retention-service-unavailable`), the power monitor and the optional `FOREGROUND_ACTIVITY_TRACKER` PER RUN (no tracker → `Infinity`). The service runs the lifecycle step inside the same budget as observation retention, and the summary reports `purged <p> processed, quarantined <q> stuck, archived <a> / deleted <d> / evicted <e> memories, reclaimed <r> pages`. It maps `skipped` → skipped outcome, `completed`/`partial` → summary, and THROWS on `failed` with the reason token only, so the run row records `failed` — `MemoryRetentionService.run` itself never rejects. Registration runs no SQL beyond the upsert.
- **The skills backlog cleanup job is also one spec, two registrations.** `SKILL_BACKLOG_CLEANUP_JOB` (`@ptah/skills-backlog-cleanup`, handler `skills:backlog-cleanup`, `41 * * * *` UTC) and its handler live here. Both hosts guard registration on `SKILL_BACKLOG_CLEANUP_SERVICE` plus `handlerRegistry.has`; registration only upserts the job. Each run resolves the service and power monitor afresh, maps a skip reason directly, throws only a failed reason token, and reports examined/rejected/kept/invocation-deletion/deferred-error/root-unknown counters plus the per-run no-transcript rejection subset.
