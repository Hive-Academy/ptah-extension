...
- **A stop request outranks `force` (TASK_2026_271, verification review)** — `AdapterLifecycleService` keeps a `stopping` set of platforms the operator (or shutdown) asked to be DOWN. Cancelling a reconnect only clears a PENDING timer; once the timer fires, its callback deletes itself from `reconnectTimers` and runs `reconnect()` as a chain nobody holds a handle to, and `maybeStart(platform, true)` walks straight past the enable flag a concurrent `stopPlatform()` just cleared. `stopPlatform` / `cancelAllReconnects` (the first move of `stop()`) therefore ADD to `stopping` before anything awaits, and `maybeStart` + `startAdapter` — the last gate before the network `login()` — both refuse to start a platform in that set **even under `force: true`**. Only an explicit `startPlatform` / `startEnabled` clears it. Without this, a stop landing mid-reconnect brought the adapter back after the caller believed it stopped, and at shutdown leaked a live connection past `will-quit`.
