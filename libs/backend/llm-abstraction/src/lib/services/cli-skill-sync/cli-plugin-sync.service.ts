 ...
 * Evidence: Services registered as singletons via container.registerSingleton()
 * (llm-abstraction/di/register.ts:84-93).
 *
...
export class CliPluginSyncService {
  ...
  async syncOnActivation(
    ...
    for (const cli of installedClis) {
      try {
        ...
        // Perform sync
        const status = await this.syncForCli(cli, pluginPaths, enabledPluginIds);
        results.push(status);
      } catch (error) {
  ...
  /**
   * Remove all Ptah skills from all CLIs.
  ...
  async cleanupAll(): Promise<void> {
    ...
    for (const [cli, installer] of this.installers) {
      try {
        ...
        this.logger.warn(`[CliPluginSync] Cleanup failed for ${cli}`, {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    }

    // Also remove agent files from CLI directories
  ...
  async removeCliAgents(clis: CliTarget[]): Promise<void> {
    ...
    for (const cli of clis) {
      try {
        ...
        // Remove files that were generated by Ptah
        // We can't easily distinguish Ptah-generated files from user-created ones
        // without a manifest. For now, we leave agent files in place.
        // A future enhancement could track generated agent file paths.
        this.logger.debug(
          `[CliPluginSync] Agent cleanup for ${cli}: ${entries.length} files found`
        );
      } catch (error) {
  ...
  private resolvePluginPaths(pluginIds: string[]): string[] {
    ...
    return pluginIds
      .map((id) =>
        join(this.extensionPath!, 'assets', 'plugins', id)
      )
      .filter(Boolean);
  }
