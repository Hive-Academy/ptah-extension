/**
 * `electron-command-registry.spec.ts` — runs `runCommandRegistryContract`
 * against `ElectronCommandRegistry`, plus Electron-specific coverage of
 * `getRegisteredCommands` (used by the menu builder).
 */
...
runCommandRegistryContract(
  'ElectronCommandRegistry',
  () => new ElectronCommandRegistry(),
);

describe('ElectronCommandRegistry — Electron-specific behaviour', () => {
  let registry: ElectronCommandRegistry;

  beforeEach(() => {
    registry = new ElectronCommandRegistry();
  });
