...
Interfaces: `IMemoryReader`, `IMemoryLister`, `ICuratorLLM` (+ `CuratorCallOptions`), `ICompactionCallbackRegistry`, `ISymbolSink`, `IKnowledgeAgent`.
...
- `src/lib/curator-llm.port.ts` — `ICuratorLLM` (consumed by curator; implemented in `agent-sdk/curator-llm-adapter`). `extract`/`resolve` take an optional `CuratorCallOptions { userInitiated? }` (TASK_2026_437 C14): `true` only for `memory:runNow`, which makes the adapter use the ungoverned `user-action` lane instead of `memory-curator`.
