...
function makeTempDbPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ptah-migr0018-test-'));
  return path.join(dir, 'ptah.db');
}

describe('migration 0018_corpora — schema shape (skipped without native)', () => {
  ...
  maybe('applies cleanly when memories table already exists', () => {
    ...
    try {
      expect(() => db.exec(sql0018Corpora)).not.toThrow();
    } finally {
      db.close();
    }
  });
