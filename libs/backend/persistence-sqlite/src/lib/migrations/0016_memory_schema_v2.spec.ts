...
import { sql as sql0016MemorySchemaV2 } from './0016_memory_schema_v2';

function makeTempDbPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ptah-migr0016-test-'));
  return path.join(dir, 'ptah.db');
}

describe('migration 0016_memory_schema_v2 — schema shape (skipped without native)', () => {
  ...
  maybe(
    ...
    () => {
      ...
      try {
        expect(() => db.exec(sql0016MemorySchemaV2)).not.toThrow();
      } finally {
        db.close();
      }
    },
