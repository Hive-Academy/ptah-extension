...
describe('migration 0028_gateway_conversation_workspace_root — registry entry', () => {
  ...
  it('is the highest bundled version', () => {
    // Bumped to 34 when TASK_2026_180 appended 0034_skill_session_verdicts to
    // the MIGRATIONS tuple (33 before that, and 32 before that, for the same
    // task's phase 1 and phase 0). This assertion tracks the current highest
    // version and moves forward with every appended migration (see 0027 → 0028
    // precedent).
    expect(Math.max(...MIGRATIONS.map((m) => m.version))).toBe(34);
  });
});
