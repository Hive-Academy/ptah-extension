...
**Handler classes** (≈30):
Tier 1: `SessionRpcHandlers`, `ContextRpcHandlers`, `AutocompleteRpcHandlers`, `SubagentRpcHandlers`, `LlmRpcHandlers`, `PluginRpcHandlers`, `PtahCliRpcHandlers`, `BootRpcHandlers`.
...
- **`boot:getReadiness` has `requires: []` and never throws.** It is the call the renderer makes to find out whether the backend is usable, so it cannot itself depend on a capability or fail: any error path falls back to `{ readiness: 'ready', phase: 'settled' }`, which dismisses the boot screen rather than stranding the user behind it.
...
- **`SessionMcpStatusRegistry` (`src/lib/chat/session/`) is the read side of `session:status`'s `mcpServers` / `notices` fields (TASK_2026_375).** A bounded LRU (256) keyed by session id, fed by `ChatSessionService` subscribing to the two agent-sdk producers, with `rekey` wired to `SessionIdResolvedCallbackRegistry` like `SessionTurnStateRegistry`. `rekey` MERGES rather than overwrites: the stderr notice arrives under the tabId before the SDK `init` message can re-key to the real session id, and an empty server list at the real id does not beat a populated placeholder list. `McpDirectoryRpcHandlers` gained four methods for the Smithery Connections API — `smitheryAccount` (namespace), `listSmitheryConnections` (per-connection status + `managedByPtah`), `smitheryConnectionStatus` (poll target), and `openSmitherySetup` (re-PUTs the connection for a fresh single-use `setupUrl`, then opens it) — all under the already-allowed `mcpDirectory:` prefix.
