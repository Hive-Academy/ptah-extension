...
- **Every `.mcp.json` ownership mutation goes through `CodeExecutionMCP`'s operation queue, and `withMcpConfigLock` is not a substitute for it.** The lock is `harness-sync`'s and is keyed per FILE; the races here span DIFFERENT files. A user switching workspace A → B → C produces two re-points that both read `registeredMcpJsonPath` as A and then write B and C independently, stranding a live `ptah` entry in B for a server that no longer serves it; a `stop()` right after a switch unregisters A while an outstanding event writes B back with a port that is about to die. So `mcpOpQueue` serializes the OPERATION (a job reads the registration record only after the previous job finished updating it) and `repointGeneration` makes the newest request win (a superseded re-point is dropped before it touches disk). `stop()` bumps the generation and sets `stopped`, which is what cancels an in-flight or late-arriving re-point. Both halves are needed and both are pinned by `http-mcp-server.service.spec.ts` section 6 — with a DELIBERATELY DEFERRED lock, because the straight-through lock stub completes each mutation inside one microtask and neither race is reachable without it. The real-lock, real-`fs` half lives in `http-mcp-server.service.concurrency.spec.ts` (TASK_2026_332).
