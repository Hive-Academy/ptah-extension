...
- **The Codex `features` flag is written alongside the entry, into the SAME file, and is best-effort.** Codex has historically kept MCP tools out of the model's tool list until it runs a tool search, and `features.tool_search_always_defer_mcp_tools = false` is what `CodexCliAdapter` sends in-process to stop that. `applyTargetSideEffects` writes the file-based half at `slot.configPath` (so it lands at the same scope as the server entry) and `clearTargetSideEffects` gives it back. **Measured on codex-cli 0.150.1, that flag now reports stage `removed` with effective state `true`, and neither `-c features.tool_search_always_defer_mcp_tools=false`, nor `--disable`, nor the key in a config file moved it.** It is written anyway because it is fenced, reversible and free — but do not claim it is what makes `ptah_*` tools appear, and re-measure before relying on it. Never fatal: the server entry is correct without it.
