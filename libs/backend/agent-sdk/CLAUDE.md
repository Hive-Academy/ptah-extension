...
- **Three read caches, each with an EXACT validity token — never a bare TTL (TASK_2026_353).** `SdkModelService` keys its catalog on an auth fingerprint (active auth method + base URL + hashed credential + tier `ANTHROPIC_DEFAULT_*_MODEL` values) read from the process-global `AuthEnv` the auth strategies mutate in place, so a provider switch misses by construction rather than by someone remembering to call `clearCache()`; `clearCache()` stays, and bumps a `cacheGeneration` that every write-back re-checks, so a fetch already in flight when auth changed cannot write the pre-change catalog back. `JsonlReaderService.findSessionsDirectory` memoises per workspace against the `~/.claude/projects` `mtimeMs` — a directory's mtime moves when a child is added or removed and at no other time, which is exactly what that answer depends on, and is why the NEGATIVE answer is cached too. `JsonlReaderService.readJsonlMessages` memoises the parse on `(path, size, mtimeMs)`, needing no extra syscall because the size guard already stats; `readJsonlTail` is deliberately excluded (its result also depends on `maxBytes`, and its callers read a moving window of a file being appended to). Each cached parse is handed out as a fresh array so an array-level mutation by one caller cannot reach the next. If you add a fourth, bring a token — a TTL alone lets a stale answer be served for its whole window, which is the failure mode all three of these avoid.
