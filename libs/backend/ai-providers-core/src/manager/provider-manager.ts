...
// import { PROVIDER_MESSAGE_TYPES } from '@ptah-extension/shared'; // DELETED - Phase 0 purge
// import type { EventBus } from '@ptah-extension/vscode-core'; // DELETED - Phase 0 purge
...
export class ProviderManager implements IProviderManager {
  ...
  constructor(
    // @inject(TOKENS.EVENT_BUS) private readonly eventBus: EventBus, // TODO: Phase 2 RPC - remove EventBus
    ...
    // this.setupEventListeners(); // TODO: Phase 2 RPC - remove EventBus
  ...
  registerProvider(provider: EnhancedAIProvider): void {
    ...
    // this.eventBus.publish(PROVIDER_MESSAGE_TYPES.AVAILABLE_UPDATED, { // TODO: Phase 2 RPC - remove EventBus
  ...
  async selectBestProvider(
    ...
    // if (previousProviderId !== result.providerId) { // TODO: Phase 2 RPC - remove EventBus
  ...
  async switchProvider(providerId: ProviderId): Promise<boolean> {
    ...
    // if (previousProviderId !== providerId) { // TODO: Phase 2 RPC - remove EventBus
    //   this.eventBus.publish(PROVIDER_MESSAGE_TYPES.CURRENT_CHANGED, {
    //     from: previousProviderId,
    //     to: providerId,
    //     reason: 'user-request',
  ...
  on(
    ...
    // TODO: Phase 2 RPC - remove EventBus
  ...
  private async updateAllProviderHealth(): Promise<void> {
    ...
    for (const [providerId, provider] of this.providers.entries()) {
      try {
        ...
        // if (previousHealth && previousHealth.status !== health.status) { // TODO: Phase 2 RPC - remove EventBus
  ...
  private async handleProviderFailure(
    ...
    if (availableProviders.size === 0) {
      ...
      // this.eventBus.publish(PROVIDER_MESSAGE_TYPES.ERROR, { // TODO: Phase 2 RPC - remove EventBus
    ...
    try {
      ...
      // this.eventBus.publish(PROVIDER_MESSAGE_TYPES.CURRENT_CHANGED, { // TODO: Phase 2 RPC - remove EventBus
      //   from: failedProviderId,
      //   to: result.providerId,
      //   reason: 'auto-fallback',
      //   timestamp: Date.now(),
