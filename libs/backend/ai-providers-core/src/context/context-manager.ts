/**
 * Context Manager Service - AI Provider Context Management
 * Extracted from apps/ptah-extension-vscode/src/services/context-manager.ts
 *
 * Manages file context for AI providers with intelligent optimization,
 * caching, and workspace-intelligence integration.
 */
 ...
 *
 * Manages file inclusion/exclusion for AI providers with intelligent optimization,
 * integrated with workspace-intelligence services for token counting, relevance scoring,
...
export class ContextManager implements vscode.Disposable {
  ...
  private readonly CHARS_PER_TOKEN = 4; // Rough estimate, TokenCounterService provides accurate counts
  ...
  constructor(
    ...
    this.logger.info(
      'ContextManager initialized with workspace-intelligence integration'
    );
  }
  ...
  getCurrentContext(): ContextInfo {
    const tokenEstimate = 0; // Will be calculated async, placeholder for now
  ...
  async getTokenEstimateAsync(): Promise<number> {
    // Use workspace-intelligence TokenCounterService for accurate counting
    const files = Array.from(this.includedFiles);
    ...
    // Batch count all file contents
    return this.tokenCounter.countTokens(contents.join('\n'));
  }

  getTokenEstimate(): number {
    // Synchronous estimate using rough calculation
    // For accurate async count, use getTokenEstimateAsync()
    let totalChars = 0;
  ...
  getOptimizationSuggestions(): OptimizationSuggestion[] {
    ...
    if (currentTokens > this.MAX_TOKENS * 0.8) {
      // Use workspace-intelligence services for intelligent suggestions
      ...
      // Note: ContextSizeOptimizerService could be used for advanced optimization
      // This would require making getOptimizationSuggestions() async
      // For now, keep simple suggestions. Advanced optimization can be triggered separately.
    }
  ...
  private async estimateTokenSavingsAsync(files: string[]): Promise<number> {
    const contents: string[] = [];
  ...
  private async loadFromWorkspaceState(): Promise<void> {
    try {
      ...
      this.logger.error('Failed to load context state', error as Error);
    }
  }
  ...
  private async saveToWorkspaceState(): Promise<void> {
    try {
      ...
      this.logger.error('Failed to save context state', error as Error);
    }
  }
  ...
  async getAllFiles(
    ...
    try {
      ...
      this.logger.error('Failed to get all files', error as Error);
      return [];
    }
  }
  ...
  /**
   * Private helper methods for enhanced file search
   */
  ...
  dispose(): void {
    this.logger.info('Disposing Context Manager...');
