...
import { FileSystemManager, FileOperationOptions, FileWatcherConfig } from './file-system-manager';
...
describe('FileSystemManager - User Requirement: VS Code File System Abstraction', () => {
  ...
  beforeEach(() => {
    ...
    mockWorkspace.createFileSystemWatcher.mockReturnValue(mockFileSystemWatcher);
  ...
  describe('User Scenario: File Reading Operations', () => {
    ...
    it('should handle read errors and track them', async () => {
      ...
      await expect(fileSystemManager.readFile(testUri)).rejects.toThrow('File not found');
  ...
  describe('User Scenario: File Writing Operations', () => {
    ...
    it('should handle write errors properly', async () => {
      ...
      const content = new Uint8Array([1, 2, 3]);
      await expect(fileSystemManager.writeFile(testUri, content)).rejects.toThrow('Permission denied');

      // THEN: Should publish error with proper categorization
      expect(mockEventBus.publish).toHaveBeenCalledWith('error', expect.objectContaining({
  ...
  describe('User Scenario: File Deletion Operations', () => {
    it('should delete files and directories with tracking', async () => {
      ...
      expect(mockWorkspace.fs.delete).toHaveBeenCalledWith(
        testUri,
  ...
  describe('User Scenario: File Copy and Move Operations', () => {
    it('should copy files with comprehensive tracking', async () => {
      ...
      expect(mockWorkspace.fs.copy).toHaveBeenCalledWith(
        testUri,
        targetUri,
        { overwrite: false }
      );
    ...
    it('should move files with proper tracking', async () => {
      ...
      expect(mockWorkspace.fs.rename).toHaveBeenCalledWith(
        testUri,
        targetUri,
        { overwrite: true }
      );
  ...
  describe('User Scenario: File Information and Directory Listing', () => {
    ...
    it('should read directory contents with filtering', async () => {
      ...
      const result = await fileSystemManager.readDirectory(testUri, { includeHidden: false });
  ...
  describe('User Scenario: Error Handling and Categorization', () => {
    it('should categorize different error types properly', async () => {
      const errorTests = [
        ...
        { error: 'EACCES: permission denied', expectedCode: 'PERMISSION_DENIED' },
    ...
    it('should validate URIs properly', async () => {
      // GIVEN: Invalid URI
      const invalidUri = { scheme: '', fsPath: '', path: '', toString: () => '' } as vscode.Uri;

      // WHEN: Attempting operation with invalid URI
      await expect(fileSystemManager.readFile(invalidUri)).rejects.toThrow('Invalid URI');
    });
  });
  ...
  describe('User Scenario: Manager Lifecycle', () => {
    ...
    it('should handle disposal errors gracefully', () => {
      ...
      expect(mockEventBus.publish).not.toHaveBeenCalledWith('analytics:trackEvent', expect.objectContaining({
        event: 'fileSystem:managerDisposed'
      }));
    });
  });
