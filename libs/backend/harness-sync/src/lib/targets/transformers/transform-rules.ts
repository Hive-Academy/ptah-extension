 ...
 *
 * Shared transformation rules and regex patterns used by the per-CLI agent
 * transformers. Pure functions with no I/O and no DI.
 *
 * Moved verbatim from `agent-generation/services/cli-agent-transforms/` in
 * TASK_2026_278 Batch 2 — see `agent-transformer.port.ts` for why. Its spec
 * moved alongside it unchanged.
 *
...
> = {
  ...
  // surface. `AntigravityTarget` reports `agents: 'unsupported'` because agy
  // documents no subagent format at all, so these values stay unexercised until
  // one appears — at which point a transformer has its vocabulary ready.
  antigravity: {
    askUser: 'ask the user directly in your response',
