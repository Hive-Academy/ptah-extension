/**
 * `cli-token-counter.spec.ts` — runs `runTokenCounterContract` against
 * `CliTokenCounter` (gpt-tokenizer BPE). Beyond the contract we assert:
 *
 ...
 *   - Fallback path (encode throws → heuristic `ceil(len/4)`) never causes
 *     the public surface to reject.
 */

import 'reflect-metadata';
import { runTokenCounterContract } from '@ptah-extension/platform-core/testing';
import { CliTokenCounter } from './cli-token-counter';

runTokenCounterContract('CliTokenCounter', () => new CliTokenCounter());

describe('CliTokenCounter — CLI-specific behaviour', () => {
  let counter: CliTokenCounter;

  beforeEach(() => {
    counter = new CliTokenCounter();
  });
