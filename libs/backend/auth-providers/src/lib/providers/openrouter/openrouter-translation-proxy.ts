 ...
 * OpenAI Chat Completions protocol, so the SDK â†’ proxy â†’ OpenRouter flow is:
 *
 *   Claude Agent SDK (Anthropic Messages)
 *     â†’ local proxy on 127.0.0.1:ephemeral-port
 *     â†’ translate to OpenAI Chat Completions
 *     â†’ https://openrouter.ai/api/v1/chat/completions
 *     â†’ translate streaming response back to Anthropic SSE
 *     â†’ SDK
 *
...
export class OpenRouterTranslationProxy extends TranslationProxyBase {
  ...
  /**
   * OpenRouter's API endpoint is static â€” no auth-state-driven overrides.
   */
  ...
  /**
   * On 401, OpenRouter keys cannot be refreshed â€” they're user-provided and
  ...
  protected async onAuthFailure(): Promise<boolean> {
    this.logger.error(
      '[OpenRouterProxy] Got 401 from OpenRouter â€” the API key is invalid, ' +
   ...
   * No normalization/mapping needed â€” the model ID is passed through as-is.
   */
  ...
  /**
   * OpenRouter has no static model list â€” the full catalog (200+ models) is
