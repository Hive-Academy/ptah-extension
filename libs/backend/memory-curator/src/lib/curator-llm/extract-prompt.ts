/**
 * Extract prompt — instructs the curator LLM to surface durable knowledge
 * from a transcript window as a structured JSON array.
 *
 * The schema is intentionally narrow so a small model (Haiku/8B-class) can
 * comply reliably. Output is parsed by `SdkInternalQueryCuratorLlm`.
 */
