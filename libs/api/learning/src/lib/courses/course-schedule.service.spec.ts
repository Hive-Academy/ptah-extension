...
function wire(): Wired {
  ...
  prisma.course.findFirst.mockResolvedValue({ id: 'course-1', slug: 'cohort-1' });
...
describe('apply — writes only what changed, audits exactly once', () => {
  ...
  it('everything runs in ONE transaction', async () => {
    ...
    await service.schedule(
      { ...COHORT_1, confirmModuleCount: 3, confirmLastReleaseDate: '2026-09-03' },
      true,
      audit,
    );
...
describe('refusals that are not the echo', () => {
  it('a missing or soft-deleted course is a 404, and nothing is read after it', async () => {
    ...
    await expect(
      service.schedule(
        { ...COHORT_1, confirmModuleCount: 10, confirmLastReleaseDate: LAST_RELEASE_DATE },
        true,
        audit,
      ),
...
describe('C4 reusability — cohort 2 and 3 need no code change', () => {
  it('a TWELVE-module course gets twelve dates, from the rows and not a literal', async () => {
    ...
    const result = await service.schedule(
      { ...COHORT_1, confirmModuleCount: 12, confirmLastReleaseDate: '2026-09-16' },
      true,
      audit,
    );
  ...
  it('a course with ONE module works too', async () => {
    ...
    const result = await service.schedule(
      { ...COHORT_1, confirmModuleCount: 1, confirmLastReleaseDate: '2026-09-01' },
      true,
      audit,
    );
