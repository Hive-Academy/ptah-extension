...
function parseCivilDate(isoDate: string): number {
  ...
  if (!match) {
    throw new ScheduleInputError(
      `startDate must be YYYY-MM-DD, received ${isoDate}`,
    );
  }
  ...
  if (
    ...
    throw new ScheduleInputError(
      `startDate ${isoDate} is not a real calendar date`,
    );
  }
  return civil;
}
...
function localWallClockToInstant(
  ...
  const offsetBefore =
    wallClockAsCivil(target - MS_PER_DAY, format) - (target - MS_PER_DAY);
  const offsetAfter =
    wallClockAsCivil(target + MS_PER_DAY, format) - (target + MS_PER_DAY);
