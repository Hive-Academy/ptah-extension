import { ScheduleInputError, computeWeekdaySchedule } from './weekday-schedule';
...
function dated(input: Parameters<typeof computeWeekdaySchedule>[0]): string[] {
...
describe('computeWeekdaySchedule — the offset tables', () => {
  ...
  it('never emits a Saturday or a Sunday, over a long run', () => {
    ...
    for (const slot of slots) {
      ...
      expect(new Date(`${slot.localDate}T00:00:00.000Z`).getUTCDay()).not.toBe(
        0,
      );
      expect(new Date(`${slot.localDate}T00:00:00.000Z`).getUTCDay()).not.toBe(
        6,
...
describe('computeWeekdaySchedule — count never assumes ten (C4 reusability)', () => {
  it('count: 1 gives exactly the start date', () => {
    expect(
      localDates({ startDate: '2026-09-01', count: 1, ...AT_0900_UTC }),
    ).toEqual(['2026-09-01']);
  });

  it('count: 3 stops inside the first week', () => {
    expect(
      localDates({ startDate: '2026-09-01', count: 3, ...AT_0900_UTC }),
