 ...
 *   - `libs/api/forum/src/lib/common/optional-field.ts`
 *   - `libs/api/learning/src/lib/common/optional-field.ts`
 *
 * A deliberate THIRD re-declaration, not an import (ASSUMPTION-11). Both
 * existing copies live in a `common/` their own module spec asserts is NOT
 * barrel-exported; widening a public barrel for two decorators is a worse trade
 ...
 * `nullable-dto.spec.ts` in this directory scans every `*.dto.ts` this batch
 * owns and fails the build on any `@IsOptional()` sitting on a field whose
 * declared type cannot be `null`, with a census of the places where it
 * legitimately can. That census is `[]` and should stay `[]`.
 */
