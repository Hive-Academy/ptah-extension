...
import { BUILDERS_CHECKOUT_ENABLED, PADDLE_CONFIG } from '@ptah-web/core';
import { isPriceIdPlaceholder } from '@ptah-web/core';
...
export class PricingGridComponent implements OnInit, OnDestroy {
  ...
  protected readonly buildersCheckoutEnabled = inject(
  ...
  public readonly buildersCtaText = computed(() =>
    computeCtaText(this.buildersCtaVariant(), this.buildersCheckoutEnabled),
  );
   ...
   *
   * Builders checkout is gated behind `BUILDERS_CHECKOUT_ENABLED`:
  ...
  private triggerAutoCheckout(planKey: string): void {
    if (!this.buildersCheckoutEnabled) {
   ...
   * `BUILDERS_CHECKOUT_ENABLED` is false, however this was
