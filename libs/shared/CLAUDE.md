...
- `src/lib/types/rpc/` — one file per RPC namespace (agents, auth, chat, config, editor, error-codes, git, harness, indexing, memory, misc, persistence, providers, session, setup, readiness, activity). **`rpc-error-codes.types.ts` is the single source of truth for error codes across backend + frontend.** `rpc-activity.types.ts` has no `'error'` level by design — a failed run needs a channel the user cannot miss, and the cron run row is that channel. **`rpc-degradation.types.ts` (TASK_2026_383) lives in this folder but is NOT an RPC namespace** — it is push-only: `DegradationEventPayload` + the closed `DegradationSource` / `DegradationSeverity` unions + `isDegradationEventPayload`, broadcast as the `degradation:event` message and never requested. There is no `degradation:` method, and nothing was added to `ALLOWED_METHOD_PREFIXES`. Widening `DegradationSource` means appending to the union **and** to `DEGRADATION_SOURCE_VALUES` together; the guard drops a payload carrying an unknown source.
- **`chat:resume` carries one transcript (TASK_2026_437 C15).** `ChatResumeResult.events` (`rpc-chat.types.ts`) is the only history in the reply; it is the tail page when `historyPage` is requested, and older pages come through `chat:history-page`. Without `historyPage`, the full-history response is unchanged. The text-only `messages` projection was deleted end to end, and `rpc-chat.types.spec.ts` fails at compile time if the key comes back. `SessionLoadResult.messages` / `agentSessions` (`rpc-session.types.ts`) are typed `[]` and have always been empty — `session:load` validates metadata only. A reader that needs the transcript replays `ChatResumeResult.events`.
