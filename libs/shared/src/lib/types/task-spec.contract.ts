...
One sub-folder per task, named \`TASK_YYYY_NNN_xxxx\`. **The folder name is the
...
Scan \`${SPEC_ROOT}\` on \`origin/main\` (run \`git fetch\`, then \`git ls-tree\`),
every path from \`git worktree list\`, and the local folder. Take the highest
\`NNN\` for the current year, add one, zero-pad to at least three digits, and
append an underscore plus four random lowercase hex characters
(\`TASK_YYYY_NNN_xxxx\`). Claim the folder with an exclusive, fail-if-exists
\`mkdir\`; it is the lock. Never derive the next id from \`registry.md\` — that
file is generated and can be stale. Never rename an existing folder.
`;
}
