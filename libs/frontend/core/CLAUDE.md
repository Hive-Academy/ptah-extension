...
- `src/lib/services/message-router.service.ts` — single `window.addEventListener('message', ...)`, builds `Map<messageType, MessageHandler[]>` from all `MESSAGE_HANDLERS` multi-provider entries at construction. **Burst coalescing (TASK_2026_437 C18, INV-11)**: the listener is attached outside Angular and only queues; one `scheduleMacrotask` drain dispatches the whole queue in arrival order inside ONE `ngZone.run`, so a burst after a main-process stall costs one zone entry and one change-detection pass, not one per message. Works unchanged under the no-op `NgZone` of a zoneless host. A drain takes only what was queued when it started; messages arriving during it get the next drain. **`rpc:response` is flushed synchronously in its own task (R-P8)**, together with every push queued ahead of it, and the listener uses the **capture phase** so that flush runs before `rpc-call.util.ts`'s own bubble-phase listener resolves the same response — otherwise an awaiting caller resumes before earlier pushes landed. Do not drop `{ capture: true }` or turn the flush into a deferred drain. A BATCH carrying an `rpc:response` gets the same flush and is reported once as a producer regression (producers never batch responses). Every dispatched message, each BATCH member included, runs in its own try/catch reported to `ErrorHandler`; a malformed BATCH envelope and a failed drain post are reported too (the latter drains synchronously instead of wedging the queue). Teardown removes the listener, cancels the pending drain and drops the queue. Without `MessageChannel` (jsdom only) each message drains in the task it arrived in, which is why the app-level routing specs can assert delivery synchronously; a spec that needs the coalescing installs its own `MessageChannel`
- `src/lib/services/macrotask-scheduler.ts` — `scheduleMacrotask(callback): MacrotaskHandle` and `yieldToMacrotask(): Promise<void>`, the ONE way renderer code yields a macrotask turn. A `MessageChannel` message, never `setTimeout` (clamped, and throttled to ~1 s in a hidden Electron window) or `requestAnimationFrame` (never fires for a hidden window). Without `MessageChannel` (jsdom) the callback runs synchronously and the yield resolves on a microtask; the global is read per call, so a spec-installed stub is honoured. A failed post closes the channel and rethrows
...
- `src/lib/services/boot-status.service.ts` — the renderer's one source of truth for boot progress. Pulls `boot:getReadiness` once on construction and then follows `boot:readinessChanged` pushes; re-pulls every 2 s while `warming` (the host's push is edge-triggered and its emit is failure-swallowing, so a dropped transition would otherwise strand the user behind the boot screen). **The `ready` default is load-bearing**: the same bundle runs in the VS Code webview, where no `boot:` RPC is registered and no push ever arrives — every degrade path leaves the service reporting `ready`, i.e. never blocking. `isBlockingBoot` = `warming` AND a phase before `harness`
- `src/lib/services/back-office-activity.service.ts` — ten push message types (indexing, memory, skill synthesis, embedder, vec, boot) folded into one 50-item newest-first ring for the passive ticker. Coalescing is required, not an optimisation: `indexing:progress` is per-file, so an item whose `source:kind` matches the newest one within 750 ms replaces it in place, keeping its slot and id. Pure sink, no host branch — `isIdle` settles itself off a wall-clock signal
