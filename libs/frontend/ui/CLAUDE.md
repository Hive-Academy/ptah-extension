...
- `src/lib/native/` — recommended Floating-UI-based primitives
  ...
  - `provider-model-picker/` — `ProviderModelPickerComponent`: two `<select>`s (provider from **`getAllAnthropicProviders()`** — built-ins PLUS user-defined; model from the injected `PROVIDER_MODELS_LOADER`), each with an `''` "inherit" sentinel. **Never iterate `ANTHROPIC_PROVIDERS` here**: the picker still renders, so the only symptom is a user's own provider missing from this one surface while working everywhere else. Pinned by a spec that registers a custom entry and asserts it appears. Inputs `provider` / `model` / `label` / `defaultTier` / `requiresToolUse`; output `selectionChange`. Surfaces `ProviderModelInfo.supportsToolUse` as a warning and `contextLength` as a suggested max-input hint
