      ...
      <div class="flex items-start gap-2">
        ...
        @if (summary().failures.length > 0) {
          ...
          <button
            ...
            (click)="retry.emit(summary().status)"
          >
...
export class TaskBulkSummaryComponent {
  ...
  /** Re-run the target status over the still-selected failures (FR-C4.8). */
  public readonly retry = output<TaskStatus>();
  ...
  protected readonly headline = computed(() => {
    ...
    const target = TASK_STATUS_LABELS[summary.status];
    const failed = summary.failures.length;
    if (failed === 0) {
      return `${summary.succeeded} task(s) moved to ${target}.`;
    }
    return `${summary.succeeded} task(s) moved to ${target}; ${failed} were refused and are still selected.`;
