 ...
 *    the strip must attribute dispatches to the stage that spent them before
 *    anyone tunes the tiers.
 *
...
describe('SkillPipelineStatusComponent — per-stage cost (R3)', () => {
  ...
  it('shows the workspace-wide dispatch total beside the queued row count', () => {
    ...
    expect(text).toContain('5 dispatches / 2 queued');
  });
  ...
  it('says nothing is queued rather than rendering an empty strip', () => {
    const { root } = mount();
    expect(stageNodes(root).length).toBe(0);
    expect(
      root.querySelector('[data-testid="skills-stage-cost-empty"]')
        ?.textContent,
