...
@Component({
  ...
  template: `
    <div class="flex flex-col gap-2">
      ...
      @if (agentInfo().hasSummary || agentInfo().summaryContent) {
        <div class="border border-base-300/50 rounded-lg overflow-hidden">
          ...
          >
            ...
            />
            <lucide-angular [img]="FileTextIcon" class="w-3.5 h-3.5 text-purple-400" />
            <span class="text-[11px] font-medium text-base-content/70">Summary</span>
            @if (!agentInfo().summaryContent && agentInfo().hasSummary) {
              <lucide-angular [img]="LoaderIcon" class="w-3 h-3 text-base-content/40 animate-spin ml-auto" />
            }
          </button>
          ...
          @if (!summaryCollapsed()) {
            <div class="px-2.5 py-2 max-h-64 overflow-y-auto">
              @if (agentInfo().summaryContent) {
                ...
                <div class="flex items-center gap-2 text-[11px] text-base-content/40 italic">
        ...
        <div class="border border-base-300/50 rounded-lg overflow-hidden">
          ...
          >
            <lucide-angular
              ...
              class="w-3.5 h-3.5 text-base-content/50"
            />
            <lucide-angular [img]="WrenchIcon" class="w-3.5 h-3.5 text-blue-400" />
            <span class="text-[11px] font-medium text-base-content/70">
              Execution
              @if (executionTree()?.children?.length) {
                <span class="text-base-content/40 ml-1">({{ executionTree()?.children?.length }} tools)</span>
              }
            </span>
          ...
          @if (!executionCollapsed()) {
            <div class="px-2.5 py-2 max-h-96 overflow-y-auto">
              @if (hasExecutionNodes()) {
                @for (child of executionTree()?.children || []; track child.id) {
      ...
      @if (!agentInfo().hasSummary && !agentInfo().summaryContent && !agentInfo().hasExecution && !hasExecutionNodes()) {
