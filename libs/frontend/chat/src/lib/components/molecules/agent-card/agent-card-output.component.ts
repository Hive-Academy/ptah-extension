  ...
  template: `
    ...
    >
      <div class="p-2 space-y-1.5">
        @for (segment of segments(); track $index) { @switch (segment.type) {
        @case ('heading') {
        <div class="flex items-center gap-1.5 mt-2.5 mb-1 first:mt-0">
          <span class="w-1 h-3 rounded-full bg-warning/70 flex-shrink-0"></span>
          <span
        ...
        <div class="bg-error/5 rounded border border-error/15 overflow-hidden">
          <div class="flex items-center gap-1.5 px-2 py-0.5">
...
$ {{ segment.toolName }}</pre
        ...
        } } } } @for (seg of stderrSegments(); track $index) { @if (seg.type ===
        'error') {
        <div class="bg-error/10 rounded px-2 py-1 border border-error/20 mt-1">
          <pre
            class="text-[10px] font-mono text-error whitespace-pre-wrap break-words m-0 leading-relaxed"
            >{{ seg.content }}</pre
          >
