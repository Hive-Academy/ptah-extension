 ...
 *
 * TASK_RPC_HARDENING Fix 5: seeds the typed-method mechanism using the
 * existing shared `RpcMethodRegistry`. The registry already covers the
 * full method surface; callers opt in by passing a literal method name
 * instead of a `string` expression.
 */
