...
7. **`''` is never a session id** (`session-scope.ts`, TASK_2026_295). The backend could put an empty string on `FlatStreamEvent.sessionId` / `SubagentRecord.parentSessionId` before the SDK session resolved; `''` is neither nullish (so `??` skips it) nor truthy (so `!x` rejects it), which is how one agent ended up shown in every tab by one view and no tab by another. Wave 2 made both fields optional, so "not known" now travels as `undefined` and no producer has to invent a value. Every writer still normalizes through `knownSessionId` — a stored owner is a real id or `undefined`, never `''` — because `?: string` widens the type without forbidding `''`.
