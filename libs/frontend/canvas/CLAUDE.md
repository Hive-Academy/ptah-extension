...
- `canvas-layout-intent.ts` owns the `TileWidthIntent` union, 12-unit deterministic packer, preset projection, resize snapping, row-preserving reconciliation, span-aware drag projection, the 6/3/2 height tiers, and transient view-constraint projection.
...
There are three height tiers. A full tile renders at 6 height units, a compact-tall tile at 3 (`COMPACT_TALL_TILE_HEIGHT_UNITS`) and a compact tile at 2 (`COMPACT_TILE_HEIGHT_UNITS`). Both compact tiers also render at the responsive minimum width for the current capacity (4, 6, or 12 units), regardless of their stored span. The stored width intent is untouched: exiting either compact tier restores it exactly.
...
Lock is enforced in the UI and store. It blocks presets, span, row, layout focus, drag, and resize changes. View mode is the one deliberate exception: it is owned by `TabManagerService`, not by layout intent, so BOTH the header toggle and the three height-tier items in the tile menu stay enabled while locked, and the grid applies the resulting geometry through the lock exception above. The header toggle is binary — full goes to compact, and either compact tier returns to full. It deliberately does not cycle, because the affordance is one click and a cycle never round-trips. Selecting a specific tier is `setViewMode`, which the tile menu exposes. Do not add geometry back to tile intent or exceed `MAX_TILES` (`MAX_CANVAS_TILES`, currently 20 — raised from 9 in TASK_2026_471). Read the constant; never hard-code the number. Raising it again is a one-line change in `canvas-layout-intent.ts`, but the cap itself must stay: `adoptTab` returns `null` at the cap, and a caller that ignores that `null` drops its work silently.
