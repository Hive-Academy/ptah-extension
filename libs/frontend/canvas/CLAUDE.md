...
- `canvas-workspace-grid.component.ts` is the only Gridstack writer. Stable item options prevent Angular from becoming a second geometry authority; a guarded effect diffs and batches only changed nodes. It derives view constraints from `TabManagerService`, suppresses move/resize per node, and applies view-driven geometry through a narrow lock exception.
...
- Geometry flows one way: store intent plus derived view constraints → `CanvasLayoutService.computeLayout()` → `grid.update()`. Gridstack is a renderer, never a store.
...
- Drag reconstruction keeps `y` and `h` strict for every node under each candidate break mask. Named-span and compact tiles must also match `x` and `w`; full-tier auto tiles may retain their pre-drag `x`/`w` until the committed intent is projected again. No matching mask rejects the gesture. A view-mode change during a gesture cancels it and reconciles from current intent.
