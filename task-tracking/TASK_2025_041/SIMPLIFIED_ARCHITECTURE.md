...
1. ❌ **NO OAuth token storage** - Users set `ANTHROPIC_API_KEY` in VS Code settings
2. ✅ **Simple license key system** - Just verify premium status
3. ✅ **VS Code auth** - Users enter license key in extension
4. ✅ **Minimal backend** - Just license validation + Paymob webhooks
...
```typescript
// Extension reads user's API key from settings
const apiKey = vscode.workspace.getConfiguration('ptah').get<string>('anthropicApiKey');

// Use SDK with USER'S key (not ours!)
...
**KEY INSIGHT**: We NEVER store or proxy user's API key. They use their own Claude subscription directly!
    ...
    "configuration": {
      ...
      "properties": {
        ...
        "ptah.anthropicApiKey": {
          "type": "string",
          "default": "",
          "description": "Your Anthropic API key (sk-ant-...). Get it from https://console.anthropic.com/",
          "order": 2
        }
      }
    }
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (license.tier === 'premium') {
    // Register premium provider (SDK with custom tools)
    registerPremiumProvider(apiKey, context);
