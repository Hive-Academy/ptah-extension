...
- **Details**:
  - Update `entryPoints` iteration: `analysis.keyFileLocations.entryPoints ?? []`
  - Update `configs` iteration: `analysis.keyFileLocations.configs ?? []`
...
- **Details**:
  - Create new `analysis/` subdirectory if needed
  - Create standalone component with OnPush change detection
  - Input: `patterns: ArchitecturePattern[]`
  - Move lines 91-135 from analysis-results template
  - Include confidence badge and progress bar logic
- **Acceptance Criteria**:
...
- **Details**:
  ...
  - Move lines 137-265 from analysis-results template
  - Include all 6 collapse sections (entryPoints, configs, testDirectories, components, services, apiRoutes)
  - Apply null coalescing from T4.1
- **Acceptance Criteria**:
  - Component under 150 lines
...
- **Details**:
  ...
  - Move lines 301-375 from analysis-results template
  - Include error/warning badges and radial progress
- **Acceptance Criteria**:
  - Component under 150 lines
...
- **Details**:
  ...
  - Move lines 47-89 and 269-298 from analysis-results template
  - Include language distribution progress bars
- **Acceptance Criteria**:
  - Component under 150 lines
...
- **Details**:
  - Add imports for all 4 new sub-components
  - Replace inline template sections with component tags
  - Pass appropriate inputs to each sub-component
  - Keep confirmation warning and action buttons in parent
- **Acceptance Criteria**:
  - Parent component under 200 lines
...
- **Details**:
  - Update `categoryOrder` type to include `'other'`
  - Update `getAgentsByCategory` to handle 'other' category
  - Update `getCategoryLabel`, `getCategoryIcon`, `getCategoryBadgeClass` with 'other' defaults
- **Acceptance Criteria**:
...
- **Details**:
  - Update `submitAgentSelection` RPC call to return `{ success: boolean; error?: string }`
  - Check response.success before calling `setCurrentStep('generation')`
  - Display error if acknowledgment fails
- **Acceptance Criteria**:
...
- **Details**:
  ...
  - Throw error on failure (existing behavior)
- **Acceptance Criteria**:
...
**Developer**: frontend-developer (P1-3, P1-5, P1-10), backend-developer (P1-6)
**Status**: PENDING
...
- **Details**:
  ...
  - Add `urlError = signal<string | null>(null)` for error state
  - Update `onUpgradeClick` to show loading, set timeout for feedback
  - Update template with loading spinner and error alert
- **Acceptance Criteria**:
