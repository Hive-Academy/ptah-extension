...
import { FileSuggestion } from '../../core/services/file-picker.service';
 ...
 */
@Component({
  selector: 'vscode-file-suggestions-dropdown',
  standalone: true,
  imports: [CommonModule],

  template: `
    <div class="vscode-file-dropdown" [style.top.px]="positionTop" [style.left.px]="positionLeft">
      @if (isLoading) {
        ...
        <div class="vscode-file-dropdown-list">
          @for (suggestion of filteredSuggestions(); track suggestion.path; let i = $index) {
            <div
        ...
        @if (filteredSuggestions().length > maxDisplayCount) {
          <div class="vscode-file-dropdown-footer">
            Showing {{ maxDisplayCount }} of {{ filteredSuggestions().length }} files
          </div>
        }
      }
    </div>
...
export class VSCodeFileSuggestionsDropdownComponent {
  @Input() suggestions: FileSuggestion[] = [];
  @Input() searchQuery = '';
  @Input() isLoading = false;
  @Input() positionTop = 0;
  @Input() positionLeft = 0;
  @Input() maxDisplayCount = 15;

  @Output() suggestionSelected = new EventEmitter<FileSuggestion>();
  ...
  readonly filteredSuggestions = computed(() => this.suggestions.slice(0, this.maxDisplayCount));
  ...
  setFocusedIndex(index: number): void {
    this._focusedIndex.set(Math.max(0, Math.min(index, this.filteredSuggestions().length - 1)));
  }
