...
import { ChatFile } from '../../core/services/file-picker.service';
...
@Component({
  ...
  template: `
    ...
    >
      <!-- File Icon and Name -->
      <div class="vscode-file-tag-content" (click)="toggleExpanded()">
        ...
        <div class="vscode-file-tag-info">
          ...
          @if (showMetadata) {
            <div class="vscode-file-tag-meta">
              <span class="vscode-file-tag-size">{{ formatSize(file.size) }}</span>
...
export class VSCodeFileTagComponent {
  @Input({ required: true }) file!: ChatFile;
  ...
  readonly hasPreview = computed(() =>
    Boolean(this.file.preview && (this.file.type === 'image' || this.file.type === 'text')),
  );
  ...
  getFileIcon(): string {
    if (this.file.type === 'image') return '🖼️';
    if (this.file.type === 'text') return '📄';
    return '📁';
  }
  ...
  getTooltipText(): string {
    ...
    if (this.file.tokenEstimate > 0) {
      parts.push(`Tokens: ${this.formatTokens(this.file.tokenEstimate)}`);
    }

    if (this.file.isLarge) {
  ...
  getAriaLabel(): string {
    return `File ${this.file.name}, ${this.formatSize(this.file.size)}${this.file.isLarge ? ', large file' : ''}. Click to ${this.hasPreview() ? (this.isExpanded() ? 'collapse' : 'expand') + ' preview or ' : ''}remove.`;
  }
}
