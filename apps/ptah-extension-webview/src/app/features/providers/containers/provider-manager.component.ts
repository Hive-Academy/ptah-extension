...
import { ProviderService, ProviderInfo, ProviderError } from '../../core/services/provider.service';
import { LoggingService } from '../../core/services/logging.service';
import { ProviderSettingsComponent } from '../../dumb-components/providers/provider-settings.component';
...
export class ProviderManagerComponent {
  ...
  protected onProviderSelected(providerId: string): void {
    this.logger.interaction('providerSelected', 'ProviderManagerComponent', { providerId });
  ...
  protected onFallbackEnabledChange(enabled: boolean): void {
    this.logger.interaction('fallbackEnabledChange', 'ProviderManagerComponent', { enabled });
  ...
  protected onAutoSwitchEnabledChange(enabled: boolean): void {
    this.logger.interaction('autoSwitchEnabledChange', 'ProviderManagerComponent', { enabled });
  ...
  private setupProviderEventListeners(): void {
    ...
    this.providerService.onProviderHealthChange().subscribe((healthEvent) => {
      ...
      if (healthEvent.health.status === 'error') {
        this.logger.error('Provider health error', 'ProviderManagerComponent', healthEvent);
      }
    });

    // Listen for provider errors
    this.providerService.onProviderError().subscribe((errorEvent) => {
      this.logger.error('Provider error event', 'ProviderManagerComponent', errorEvent);

      // Show error notification or take recovery action
      if (!errorEvent.error.recoverable) {
        this.logger.warn('Non-recoverable provider error', 'ProviderManagerComponent', errorEvent);
      }
    });
  }
}
