...
import { LucideAngularModule, TrendingUpIcon, ActivityIcon } from 'lucide-angular';
import { type PerformanceData } from '@ptah-extension/shared';

/**
 * VS Code Dashboard Performance Chart - Pure Presentation Component
 * - Simple performance visualization with trend indicators
 * - No business logic or state management
 * - Pure VS Code styling - NO Tailwind classes
 */
@Component({
  selector: 'vscode-dashboard-performance-chart',
  standalone: true,
  imports: [CommonModule, LucideAngularModule],

  template: `
    <section class="vscode-performance-chart" [attr.aria-label]="'Performance trends'">
      <header class="vscode-chart-header">
        ...
        <div class="vscode-trend-indicators">
          <div class="vscode-trend-item" [class]="getTrendClass('latency')">
            <lucide-angular
              [img]="getTrendIcon(performanceData.latencyTrend)"
              class="vscode-trend-icon"
            ></lucide-angular>
            <span class="vscode-trend-label">Latency {{ performanceData.latencyTrend }}</span>
          </div>
          <div class="vscode-trend-item" [class]="getTrendClass('memory')">
            <lucide-angular
              [img]="getTrendIcon(performanceData.memoryTrend)"
              class="vscode-trend-icon"
            ></lucide-angular>
            <span class="vscode-trend-label">Memory {{ performanceData.memoryTrend }}</span>
          </div>
        </div>
      </header>
      ...
      @if (showHistoricalChart && hasChartData()) {
        <div class="vscode-chart-content">
          <div class="vscode-chart-placeholder">
            <div class="vscode-chart-message">
              <lucide-angular [img]="ActivityIcon" class="vscode-chart-icon"></lucide-angular>
            ...
            <div class="vscode-simple-chart">
              @for (point of getRecentDataPoints(); track point.timestamp) {
                <div
                  ...
                  [title]="'Latency: ' + point.latency + 'ms at ' + formatTime(point.timestamp)"
                ></div>
              }
            </div>
          </div>
        ...
        <div class="vscode-chart-empty">
          <div class="vscode-empty-message">
            <lucide-angular [img]="ActivityIcon" class="vscode-empty-icon"></lucide-angular>
  ...
  styles: [
    `
      ...
      .vscode-trend-item--improving {
        background-color: color-mix(in srgb, var(--vscode-terminal-ansiGreen) 15%, transparent);
        color: var(--vscode-terminal-ansiGreen);
      }

      .vscode-trend-item--stable {
        background-color: color-mix(in srgb, var(--vscode-terminal-ansiYellow) 15%, transparent);
...
export class VSCodeDashboardPerformanceChartComponent {
  @Input({ required: true }) performanceData!: PerformanceData;
  @Input() showHistoricalChart: boolean = true;
  ...
  getTrendClass(type: 'latency' | 'memory'): string {
    const trend =
      type === 'latency' ? this.performanceData.latencyTrend : this.performanceData.memoryTrend;
