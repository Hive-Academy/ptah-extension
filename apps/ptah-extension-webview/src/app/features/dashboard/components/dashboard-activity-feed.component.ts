...
/**
 * VS Code Dashboard Activity Feed - Pure Presentation Component
 * - Recent activity and event log display
 * - No business logic or state management
 * - Pure VS Code styling - NO Tailwind classes
 */
@Component({
  selector: 'vscode-dashboard-activity-feed',
  standalone: true,
  imports: [CommonModule, LucideAngularModule],

  template: `
    <section class="vscode-activity-feed" [attr.aria-label]="'Recent system activity'">
      <header class="vscode-activity-header">
        <h3 class="vscode-activity-title">Recent Activity</h3>
        <span class="vscode-activity-count">{{ activities.length }} events</span>
      </header>
      ...
      <div class="vscode-activity-content">
        @if (activities.length > 0) {
              ...
              >
                ...
                <div class="vscode-activity-content-wrapper">
                  <div class="vscode-activity-main">
                    ...
                    <p class="vscode-activity-description">{{ activity.description }}</p>
                  </div>
                  <div class="vscode-activity-meta">
                    <span class="vscode-activity-time">{{
                      formatRelativeTime(activity.timestamp)
                    }}</span>
                    <span class="vscode-activity-type" [class]="getTypeClass(activity.type)">
                      {{ activity.type }}
                    </span>
                  </div>
                </div>
          ...
          <div class="vscode-activity-empty">
            <lucide-angular [img]="InfoIcon" class="vscode-empty-icon"></lucide-angular>
  ...
  styles: [
    `
      ...
      .vscode-activity-type--system {
        background-color: color-mix(in srgb, var(--vscode-terminal-ansiBlue) 15%, transparent);
...
export class VSCodeDashboardActivityFeedComponent {
