...
// Child Components
import { SessionSelectorComponent } from '../../dumb-components/session/session-selector.component';
import {
  SessionCardComponent,
  SessionAction,
} from '../../dumb-components/session/session-card.component';
...
/**
 * Session Manager Component - Smart Component
 * Orchestrates session management UI with backend integration
...
export class SessionManagerComponent implements OnInit, OnDestroy {
  ...
  private setupSessionSynchronization(): void {
    ...
    this.vscode.onMessage().pipe(
      ...
      const sessions = (initialData as any)?.data?.sessions || [];
    ...
    combineLatest([
      ...
      .subscribe(([currentSession, sessionsUpdate]) => {
        ...
        if (sessionsUpdate) {
          const sessions = (sessionsUpdate as any)?.data?.sessions || (sessionsUpdate as any)?.payload?.sessions || [];
          this._allSessions.set(sessions);
        }
  ...
  private async fetchAllSessions(): Promise<void> {
    try {
      ...
      this.vscode
        ...
        .subscribe((response) => {
          const sessions = (response as any)?.data?.sessions || (response as any)?.payload?.sessions || [];
          this.logger.api(
