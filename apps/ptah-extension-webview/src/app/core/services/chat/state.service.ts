...
/**
 * Chat State Service - Centralized State Management
 *
 * Responsibilities:
...
export class ChatStateService {
  ...
  updateMessage(messageId: MessageId, updates: Partial<StrictChatMessage>): void {
    const currentMessages = this._messages();
  ...
  updateClaudeMessage(messageId: MessageId, updates: Partial<ProcessedClaudeMessage>): void {
    const currentMessages = this._claudeMessages();
    const updatedMessages = currentMessages.map((msg) =>
  ...
  findClaudeMessage(messageId: MessageId): ProcessedClaudeMessage | null {
    return this._claudeMessages().find((msg) => msg.messageId === messageId) || null;
  }

  getMessagesByType(type: 'user' | 'assistant' | 'system'): StrictChatMessage[] {
  ...
  getMessagesInTimeRange(startTime: number, endTime: number): StrictChatMessage[] {
  ...
  mergeClaudeMessages(newMessages: readonly ProcessedClaudeMessage[]): void {
    ...
    // Merge new messages, updating existing ones
    newMessages.forEach((newMsg) => {
      messageMap.set(newMsg.messageId, newMsg);
    });
  ...
  upsertStreamingMessage(message: ProcessedClaudeMessage): void {
    const currentMessages = this._claudeMessages();
    const existingIndex = currentMessages.findIndex((msg) => msg.messageId === message.messageId);
