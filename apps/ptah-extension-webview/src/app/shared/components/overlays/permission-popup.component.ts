...
/**
 * VS Code Permission Popup - Pure Presentation Component
 * - Modal overlay for Claude CLI permission requests
 * - Egyptian-themed design following VS Code styling
 * - No business logic or state management
 * - Accessible with proper keyboard navigation and focus management
 */
@Component({
  selector: 'vscode-permission-popup',
  standalone: true,
      ...
      >
        ...
        <div class="vscode-modal-header">
          ...
          <vscode-action-button
            [icon]="X"
            [ariaLabel]="'Close permission dialog'"
            variant="secondary"
            (buttonClick)="onResponse('deny')"
          ></vscode-action-button>
        </div>
        ...
        <div class="vscode-modal-content">
          ...
          <div class="vscode-action-section">
            ...
            <div class="vscode-action-card">
              <div class="vscode-action-icon">
                <lucide-angular
                  [img]="getRiskIcon(permissionRequest.riskLevel)"
          ...
          <div class="vscode-risk-section">
            <div class="vscode-risk-badge" [attr.data-risk]="permissionRequest.riskLevel">
              ...
              <span class="vscode-risk-text"
                >{{ getRiskLabel(permissionRequest.riskLevel) }} Risk</span
              >
            </div>
            <p class="vscode-risk-explanation">
              {{ getRiskExplanation(permissionRequest.riskLevel) }}
            </p>
          </div>
          ...
          <div class="vscode-tool-section">
            ...
            <div class="vscode-tool-info">
              ...
              <div class="vscode-tool-timestamp">
                Requested {{ formatTimestamp(permissionRequest.timestamp) }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="vscode-modal-actions">
          <div class="vscode-action-group">
            ...
            <vscode-action-button
              [icon]="CheckCircle"
              [ariaLabel]="'Allow permission once'"
              variant="primary"
              size="small"
              (buttonClick)="onResponse('allow')"
            >
              Allow Once
            </vscode-action-button>
            ...
            <vscode-action-button
              [icon]="Shield"
              [ariaLabel]="'Always allow for this tool'"
              variant="primary"
              size="small"
              (buttonClick)="onResponse('always_allow')"
            >
              Always Allow
            </vscode-action-button>
          </div>
          ...
          <div class="vscode-action-hint">
            <p>
              Tip: Use "Always Allow" to skip future permissions for {{ permissionRequest.tool }}
            </p>
          </div>
        </div>
      </div>
  ...
  styles: [
    `
      ...
      /* Egyptian-themed accents based on risk level */
...
export class VSCodePermissionPopupComponent {
  @Input() isOpen: boolean = false;
  @Input() permissionRequest: PermissionRequest | null = null;
  @Input() allowBackdropClose: boolean = false;

  @Output() permissionResponse = new EventEmitter<PermissionResponse>();
  ...
  getRiskExplanation(riskLevel: string): string {
    switch (riskLevel) {
      case 'high':
