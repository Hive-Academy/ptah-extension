...
- **`ptah agent-cli --cli` names a TARGET; it does not ask permission, and the accepted set is DERIVED from `SYSTEM_CLI_TYPES`.** `CLI_AGENT_SELECTORS` is `[...SYSTEM_CLI_TYPES, 'ptah-cli', 'glm']`, mirroring how `agent-rpc.schema.ts` builds its zod enum, so a seventh adapter is selectable the day it is registered. Never re-list the names here. What this replaced was not a policy: `CLI_AGENT_ALLOWLIST = ['glm']` blocked all six adapters `CliDetectionService` registers (Copilot with its own permission bridge) and permitted one that is not a CLI at all. It shipped as `['glm','gemini']` when `gemini` was a real `CliType`, and the two commits that removed Gemini deleted the entry that worked and left the one that never did. The README/SKILL claim that "copilot and cursor are blocked due to Windows spawn issues" was false and is gone — do not reintroduce a softened version (TASK_2026_297).
- **`--cli` is a SELECTOR, not a `CliType`, and `agent-cli.ts` is where the two vocabularies meet.** For the six system CLIs they coincide. They do not for `glm`, an Anthropic-compatible provider (Z.AI GLM) reached through the `ptah-cli` agent type and addressed by `ptahCliId` — never a binary on PATH. The file used to bridge that gap with `allowed as unknown as CliType` (a _double_ cast, because the types do not overlap), so the only legal value was the one the backend could not route and `resume` failed with `glm CLI is not installed` for its entire life. `CLI_AGENT_TARGETS` is the single translation point and stays a `Record<CliAgentSelector, CliAgentTarget>`: the accepted set is derived, but each selector's wire MEANING must still be declared, so a future non-identity selector cannot compile until someone decides what it maps to. Do not reintroduce a cast, and do not "fix" a future selector by widening `CliType` — that would put a provider name in a binary-name union.
- **`--cli glm` is a deprecated alias for `--cli ptah-cli` and writes a notice to stderr.** Kept only because it is documented in four places, one of which ships to users (`ptah-cli-usage/SKILL.md`). The notice lives on the target (`CliAgentTarget.deprecation`) so the table stays the whole truth about a selector; it is stripped from the wire payload, which matters because `AgentResumeCliSessionParamsSchema` is `.passthrough()` and would forward a stray key untouched.
- **`--ptah-cli-id` is refused against a system CLI (exit 2).** It only means something for a `ptah-cli` target; forwarding it to a `codex` spawn would be a flag silently dropped.
- **`agent-cli resume` requires a non-empty `--task`, and sends no `ptahCliId` unless `--ptah-cli-id` was passed.** `agent:resumeCliSession` resumes a session AND hands it work; its schema puts `.min(1)` on `task`, so the old `task: opts.task ?? ''` was a producer inventing a value it did not have (the defect class TASK_2026_296 exists to remove). Omitting `ptahCliId` is load-bearing, not lazy: `resolveDefaultPtahCliId()` already picks the first enabled provider with a key and already errors actionably when there is none. Never re-implement provider resolution in the CLI.
- **`--cli` on `agent-cli stop` is optional and is a client-side check only.** `agent:stop` takes `{ agentId }`; `cli` has never reached the wire. It stays accepted and still resolved for scripts that pass it (so a typo is reported rather than echoed back unread), and is echoed into `agent_cli.stopped` only when supplied.
- **`agent-cli models list` scoped to a system CLI is a real query; scoped to `ptah-cli`/`glm` it answers `supported: false` and makes no RPC call.** `AgentListCliModelsResult` is keyed by `SYSTEM_CLI_TYPES`, so a system-scoped query is a field lookup on the same result the unscoped branch reports — no cast, and a seventh adapter comes along for free. It has no ptah-cli member, so that RPC structurally cannot answer a provider-scoped query; it previously called the RPC anyway, discarded the result and emitted a hardcoded `[]` indistinguishable from "this provider has no models". Wiring a real per-provider list requires a backend change; until then the emptiness is labelled rather than implied. The unscoped branch reports all six system CLIs (it used to drop `cursor`, `antigravity`, `opencode` and `pi`).
