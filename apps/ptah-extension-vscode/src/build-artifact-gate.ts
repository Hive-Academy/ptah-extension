 ...
 * `apps/ptah-tui/src/build-artifact-gate.ts` are near-duplicates of this
 * file. Four apps do not justify extracting a shared lib for one ~25-line
 * gate (`batches.md` Batch 4). This app has no pre-existing `support`/`config`
 * directory the way `ptah-electron` and `ptah-cli` do, so this file sits at
 * the top of `src/` beside `main.ts` and `deactivate-order.spec.ts` --
 * this app's own existing precedent for a top-level, non-nested spec.
 */
