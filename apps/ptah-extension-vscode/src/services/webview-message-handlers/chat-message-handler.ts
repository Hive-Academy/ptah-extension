  ...
  implements IWebviewMessageHandler<ChatMessageTypes>
{
    ...
    try {
      switch (messageType) {
        ...
        case 'chat:renameSession':
          return await this.handleRenameSession(payload as ChatRenameSessionPayload);
        case 'chat:deleteSession':
          return await this.handleDeleteSession(payload as ChatDeleteSessionPayload);
        case 'chat:bulkDeleteSessions':
          return await this.handleBulkDeleteSessions(payload as ChatBulkDeleteSessionsPayload);
  ...
  private async handleSendMessage(data: ChatSendMessagePayload): Promise<MessageResponse> {
    try {
      ...
      if (!currentSession) {
        ...
        const strictSession = this.sessionManager.toStrictChatSession(currentSession);
        this.sendSuccessResponse('chat:sessionCreated', { session: strictSession });
      }
      ...
      Logger.info(`Sending message to Claude CLI for session: ${currentSession.id}`);
        ...
        try {
          ...
          if (!messageResponse.success) {
            ...
            this.sendErrorResponse('chat:streamError', {
              code: 'STREAM_ERROR',
              message: messageResponse.error?.message || 'Stream error occurred',
            });
            return;
          }
  ...
  private async handleNewSession(data: ChatNewSessionPayload): Promise<MessageResponse> {
    try {
  ...
  private async handleSwitchSession(data: { sessionId: string }): Promise<MessageResponse> {
    try {
      ...
      const strictSession = session ? this.sessionManager.toStrictChatSession(session) : null;
      this.sendSuccessResponse('chat:sessionSwitched', { session: strictSession });
      return {
        requestId: CorrelationId.create(),
        success: true,
        data: { session: strictSession },
   ...
   */
  private async handleRenameSession(data: ChatRenameSessionPayload): Promise<MessageResponse> {
    try {
      const success = await this.sessionManager.renameSession(data.sessionId, data.newName);
  ...
  private async handleDeleteSession(data: ChatDeleteSessionPayload): Promise<MessageResponse> {
    try {
      const success = await this.sessionManager.deleteSession(data.sessionId);

      if (success) {
        this.sendSuccessResponse('chat:sessionDeleted', { sessionId: data.sessionId });
  ...
  private async handleBulkDeleteSessions(
    ...
    try {
      const result = await this.sessionManager.bulkDeleteSessions([...data.sessionIds]);

      // Send individual session deleted events for UI updates
      for (const deletedId of result.deleted) {
        this.sendSuccessResponse('chat:sessionDeleted', { sessionId: deletedId as SessionId });
      }
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to bulk delete sessions';
  ...
  private async handleRequestSessions(): Promise<MessageResponse> {
    try {
      ...
      this.sendSuccessResponse('chat:sessionsUpdated', { sessions: strictSessions });
  ...
  private extractFilesFromContent(content: string): string[] {
    ...
    if (uniquePaths.length > 0) {
      Logger.info(`Extracted ${uniquePaths.length} file paths from message:`, uniquePaths);
    }
  ...
  private async handlePermissionResponse(
    ...
    try {
      Logger.info(`Permission response received: ${data.response} for request: ${data.requestId}`);
      ...
      const sessionId = SessionId.safeParse(sessionIdString) || SessionId.create();
      ...
      return {
        ...
        success: true,
        data: {
          message: `Permission ${claudeResponse === 'deny' ? 'denied' : 'granted'}`,
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to handle permission response';
