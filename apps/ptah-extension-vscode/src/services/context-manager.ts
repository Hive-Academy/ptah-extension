...
import { ContextInfo, OptimizationSuggestion } from '@ptah-extension/shared';
...
export class ContextManager implements vscode.Disposable {
  ...
  async searchFiles(options: FileSearchOptions): Promise<FileSearchResult[]> {
    ...
    if (cached) {
      Logger.debug(`File search cache hit for query: ${options.query}`);
      return cached;
    }
    ...
    return new Promise<FileSearchResult[]>((resolve, reject) => {
      ...
      this.debounceState.timerId = setTimeout(async () => {
        try {
          ...
          Logger.debug(`File search completed: ${results.length} results for "${options.query}"`);
        } catch (error) {
          Logger.error('File search failed', error);

          // Reject all pending promises
          const errorToReject = error instanceof Error ? error : new Error('File search failed');
  ...
  async getAllFiles(
    ...
    if (
      ...
      return this.paginateResults(this.allFilesCache, offset, limit, includeImages);
    }

    try {
      Logger.debug('Refreshing all files cache');
      ...
      for (const file of files) {
        try {
          ...
          Logger.debug(`Skipping inaccessible file: ${file.fsPath}`);
        }
      }
      ...
      Logger.info(`Cached ${results.length} workspace files`);
   ...
   */
  async getFileSuggestions(query: string, limit = 20): Promise<FileSearchResult[]> {
  ...
  private async performFileSearch(options: FileSearchOptions): Promise<FileSearchResult[]> {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
      return [];
    }

    const { query, includeImages = false, maxResults = 100, fileTypes = [] } = options;

    // Build search pattern
    let searchPattern = `**/*${query}*`;
    if (fileTypes.length > 0) {
      const extensions = fileTypes.map((ext) => (ext.startsWith('.') ? ext.slice(1) : ext));
    ...
    const files = await vscode.workspace.findFiles(searchPattern, excludePattern, maxResults);
    ...
    for (const file of files) {
      try {
        ...
        const relevanceScore = this.calculateRelevanceScore(fileName, relativePath, query);
        ...
        results.push({
          ...
          lastModified: stat.mtime,
          isDirectory: stat.type === vscode.FileType.Directory,
          relevanceScore, // Add to interface if needed
        } as any);
      } catch (error) {
  ...
  private detectFileType(fileName: string, uri: vscode.Uri): FileSearchResult['fileType'] {
    ...
    const binaryExts = ['.exe', '.dll', '.so', '.dylib', '.bin', '.zip', '.tar', '.gz'];
  ...
  private calculateRelevanceScore(fileName: string, relativePath: string, query: string): number {
    let score = 0;
  ...
  clearFileCache(): void {
    ...
    Logger.info('File search cache cleared');
  }
