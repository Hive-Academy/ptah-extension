...
- `README.md`: Instructions to open, screen/state list, interactive features, a `## Deviations` section for asset fallbacks and departures from project tokens, a `## Lane-introduced constraints` list (only `[lane-proposed]` rules; or none), and parity cross-references to `parity-inventory.md`.
