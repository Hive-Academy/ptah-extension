 ...
 * these modules are included even though the CLI never instantiates them.
 *
 * This shim prevents runtime crashes from `import * as vscode from 'vscode'`.
 * The VS Code-specific service classes are NEVER instantiated in the CLI --
 * we register CLI-compatible replacements (CliOutputChannel, etc.) instead.
 *
 ...
 * paths should never execute in CLI mode.
 *
 * Copied from apps/ptah-electron/src/shims/vscode-shim.ts (identical logic).
 *
