/**
 * Guards every ESM bundle `apps/ptah-tui` produces.
 *
 ...
 * splits the gate per app -- this file checks ONLY `apps/ptah-tui/project.json`'s
 * own ESM target, and this app's `test` target now `dependsOn` that same
 * target (see `project.json`), so `nx affected -t test` for `ptah-tui`
 ...
 * `project.json`.
 *
 * This app declares exactly one ESM target (`build` → `tui.mjs`, output
 * beside `ptah-cli`'s bundles in `dist/apps/ptah-cli/`) and no
 * worker-shaped targets, so the "executable self-test" family from the
 * electron/cli siblings degenerates to an empty, but still asserted, set --
 * if a `build-*-worker` target is ever added here, the anti-vacuity check
 * below fails loudly rather than silently skipping it.
 *
 * Two families of assertion, both scoped to THIS app:
 *
 *   1. DISCOVERY + ANTI-VACUITY -- `apps/ptah-tui/project.json` is scanned
 *      for `@nx/esbuild:esbuild` targets with `options.format === ['esm']`.
 *      The discovered set must contain the one known target by name
 ...
 *      `createRequire(import.meta.url)`) at a lower index.
 *
 * Sits at the top of `src/` beside `main.tsx` -- this app's only other
 * top-level file (it has no `config`/`test-utils` directory the way
 * `ptah-electron` and `ptah-cli` do).
 */
...
const PROJECT_JSON_PATH = join(REPO_ROOT, 'apps', 'ptah-tui', 'project.json');
...
/** The one ESM target `apps/ptah-tui` is known to declare. */
const EXPECTED_ESM_TARGETS = ['build'] as const;

const projectConfig = loadProjectConfig();
const discoveredTargets = discoverEsmTargets(projectConfig);

describe('ptah-tui ESM bundle discovery (anti-vacuity)', () => {
...
describe.each(EXPECTED_ESM_TARGETS)(
  ...
  (targetName) => {
    ...
    const describeBundle = describeIfBuiltOrFail(
      target.bundlePath,
      `nx run ptah-tui:${target.targetName}`,
    );
