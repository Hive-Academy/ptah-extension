...
function deriveAgentName(payload: AgentProcessInfo): string {
...
export function AgentMonitor(): React.JSX.Element {
  ...
  const handleSpawned = useCallback((payload: unknown): void => {
    const data = payload as AgentProcessInfo;
  ...
  const handleOutput = useCallback((payload: unknown): void => {
    const data = payload as AgentOutputDelta;
    if (!data.agentId) return;
  ...
  useEffect(() => {
    pushAdapter.on(MESSAGE_TYPES.AGENT_MONITOR_SPAWNED, handleSpawned);
    pushAdapter.on(MESSAGE_TYPES.AGENT_MONITOR_OUTPUT, handleOutput);
    pushAdapter.on(MESSAGE_TYPES.AGENT_MONITOR_EXITED, handleExited);

    return () => {
      pushAdapter.off(MESSAGE_TYPES.AGENT_MONITOR_SPAWNED, handleSpawned);
      pushAdapter.off(MESSAGE_TYPES.AGENT_MONITOR_OUTPUT, handleOutput);
      pushAdapter.off(MESSAGE_TYPES.AGENT_MONITOR_EXITED, handleExited);
    };
