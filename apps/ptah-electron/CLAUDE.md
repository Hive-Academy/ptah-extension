...
- `src/activation/workspace-root-key.ts` — `normalizeWorkspaceRoot` + `NO_WORKSPACE_KEY`, the canonical "are these two strings the same open folder" key. Its own module because both the boot latch and the coalescer need it and `boot-heavy-services.ts` imports `plugin-activation.ts`. NOT `platform-core`'s same-named function, which resolves a path and keeps case.
- `src/activation/shutdown.ts` — `handleWillQuit` + the LIFO `disposeBootRefs` chain, split into `disposeBeforePersistence` / an awaited `stopMessagingGateway` / `disposeAfterPersistence`. A quit is deferred for either of two reasons: a post-window boot is in flight (abort, then drain for at most 2 s), or the chain has something to AWAIT — today only `GatewayService.stop()`, whose outbound drain writes to the database `SQLite close` is about to shut, bounded by `GATEWAY_STOP_BUDGET_MS`. With neither, the listener is synchronous and identical to the pre-change code.
