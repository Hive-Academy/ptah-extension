 ...
 * these specs `describe.skip` unconditionally, which is indistinguishable
 * from a pass in CI output -- exactly how a broken ESM bundle banner shipped
 * unnoticed (F-1, TASK_2026_380). Skipping is now permitted ONLY when the
 * developer opts in with `PTAH_ALLOW_SKIP_UNBUILT=1` (local dev loop without
 * a build). Without it, a missing artifact FAILS the suite with a message
 * naming the exact build command.
 *
 * `apps/ptah-cli/src/test-utils/build-artifact-gate.ts` is a near-duplicate
