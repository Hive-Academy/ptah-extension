...
| `skillSynthesis.drain.weeklyMaxItemsPerRun`  | `400`            | Weekly tier's whole-week item supply, dealt out over repeated rounds. **File-settings only — not on the RPC wire**, exactly like the nightly key above; the settings panel's "Max items per run" field does not read or write it either. Sized from a measured ~163 prefilter-eligible sessions/week × the two weekly rows (`judge-panel`, `trigger-eval`) chained off each. |
