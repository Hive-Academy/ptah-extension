...
if (dsn) {
  Sentry.init({
    ...
    // Profiling: sample 100% of transactions that have tracing enabled.
    // Combined with tracesSampleRate, this means all production requests
    // get both traces and profiles.
    profilesSampleRate: isProduction ? 1.0 : 0.1,
