...
@Component({
  ...
  template: `
    ...
    <footer
      ...
      class="pt-20 pb-10 bg-base-100"
      role="contentinfo"
    >
      <div class="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
        <div class="grid grid-cols-2 md:grid-cols-12 gap-x-8 gap-y-14 mb-20">
          ...
          @for (column of columns; track column.title) {
            <nav
              class="md:col-span-2"
              [attr.aria-label]="column.title + ' links'"
            >
              <h3 class="text-sm font-semibold text-white mb-6 tracking-wide">
                {{ column.title }}
              </h3>
              <ul class="space-y-4">
...
export class FooterComponent {
  public readonly columns: { title: string; links: FooterLink[] }[] = [
    {
      title: 'Product',
      links: [
