 ...
 * Used by both BasicPlanCardComponent and ProPlanCardComponent to eliminate code duplication.
 *
 ...
 *
 * Priority order:
 * 1. Pro subscriber viewing Basic -> 'included' (Basic card only)
 ...
 * 8. Default -> 'trial' (Basic) or 'popular' (Pro)
 *
 * @param context - Subscription context from service
 * @param planTier - The tier of the plan card ('basic' | 'pro')
...
export function computeBadgeVariant(
  ...
  if (!context) {
    return planTier === 'basic' ? 'trial' : 'popular';
  }
 ...
 * - 'upgrade': Opens checkout for Basic subscribers viewing Pro
 ...
 * - 'included': Disabled state for Pro subscribers viewing Basic
 *
 * @param context - Subscription context from service
 * @param planTier - The tier of the plan card ('basic' | 'pro')
...
export function computeCtaVariant(
  ...
  if (planTier === 'basic') {
    ...
    if (context.currentPlanTier === 'basic') {
      return 'upgrade';
    }
  }
