...
description: >-
  ...
  dialog component that displays it. So the user was shown a list of MCP servers,
  told the plugin would install them, approved, and not one byte reached
  `.mcp.json`, `~/.codex/config.toml`, `~/.copilot/mcp-config.json`,
  `.cursor/mcp.json` or `~/.gemini/config/mcp_config.json`. No intent was
  recorded, so the reconciler never saw them. That is worse than not offering the
  feature, because the user believes it happened.
---
