...
The single token is `InjectionToken<Signal<boolean>>` at `libs/frontend/core/src/lib/routing/surface-active.ts:5`. Route (a) is implemented: markdown does not inject the token and does not import core. Its webview hosts read activity and pass the current boolean to the pure pipe or the MarkdownBlockComponent signal input. This keeps the presentational library independent of router state and requires no new library, entry point, package dependency or path mapping. The rejected shared Angular entry point and its dependency/export/mapping changes have been removed; libs/shared remains platform-neutral.
...
The final implementation uses the previously requested route (a). `libs/shared/src/angular/index.ts` does not exist, shared/package.json has no Angular import to declare, and there is no shared/angular alias in tsconfig.base.json. Therefore no Angular dependency was added to shared and none of the three runtime build configs was changed. The later permission to use a shared Angular entry point was not needed to retain this working input-based design. Historical transcripts below may mention the removed entry point; those are not current source evidence.

Markdown itself performs **no injection**, optional or otherwise. Its pure pipe requires an explicit activity argument; MarkdownBlockComponent uses `active = input(true)` for website/global embeddings. However, several webview host components currently use `inject(SURFACE_ACTIVE, { optional: true }) ?? signal(true)` (for example thinking-block.component.ts:90, message-bubble.component.ts:94 and subagent-transcript-viewer.component.ts:185). **Yes: if such a host were mounted without its production provider, its markdown activity gate could silently remain active. There is no runtime warning or assertion for that fallback.** Omitting the MarkdownBlockComponent activity input similarly leaves rendering enabled.

Current composition supplies the root token at app.config.ts:128 and narrower chat/canvas element providers at app-shell.component.html:628/:599. The core surface-active.spec.ts exercises the directive injector, NgComponentOutlet and router/layout transitions; markdown's reactivation test exercises the explicit input. These tests verify the tested wiring and resume behavior, but do not universally detect a future host that omits activity wiring. Required injection in BatchedUpdateService, ExecutionNode, transcript and canvas fails loudly if its provider is absent. This does not constitute a runtime alarm for every optional markdown host.
