...
**(c) Headless CLI/TUI.** The spec makes this Ptah's easiest on-ramp: "If host does not support MCP Apps, tool behaves as standard tool (text-only fallback)" and "Tools MUST return meaningful content array even when UI is available" (`apps.mdx:389, 1558-1559`). A server (including Ptah's own, if it becomes an app author) is required (see Errata E3) to keep working as a plain text-returning tool for a client that never negotiates the `io.modelcontextprotocol/ui` extension — which describes Ptah's CLI/TUI exactly. No extra CLI-side work is needed beyond not negotiating the extension; the fallback is the server's obligation, not the headless client's.

## Work items

Host implementation (if pursued):

- Build a hand-rolled sandbox-proxy iframe (blob-URL or `srcdoc`, see Errata E2, tested empirically inside an actual VS Code webview and inside Electron) satisfying the spec's origin-separation MUST.
- Add `frame-src`/`child-src` (and `blob:`/`self` as verified, see Errata E2) to the CSP `<meta>` tag `apps/ptah-extension-webview` already emits.
...
- Provide the mandatory text-only fallback content for every UI-linked tool (see Errata E3).
...
- github.com/modelcontextprotocol/ext-apps — shallow clone of branch `main`, 2026-09-20; files read directly: `specification/2026-01-26/apps.mdx` (full), `LICENSE`, `README.md`, `package.json`, `src/app.ts`, `src/app-bridge.ts`, `src/react/*.ts`, `src/server/index.ts`
...
- `libs/backend/vscode-lm-tools/CLAUDE.md` and `apps/ptah-extension-webview/CLAUDE.md` (Ptah repo context, read-only)

## Errata (review of pull request 546)

- **E1.** Section "VS Code webview and Electron feasibility", the second bullet (the dedicated webview origin). The report says that this origin separation satisfies the spec rule that Host and Sandbox must have different origins (`apps.mdx:474`). That is wrong. The rule applies to web hosts that use the double-iframe proxy. A desktop or native host renders the View directly. VS Code creates one internal webview and sets the resource HTML in it. It does not build the intermediate proxy plus the inner iframe. Correction: the dedicated origin is VS Code-specific containment. It is not evidence that the sandbox-proxy requirement is satisfied.
- **E2.** The same section, constraint 1 and constraint 2 (the `blob:` workaround). Also "Work items", host items 1 and 2. The "Unknowns" section has no `blob:` entry. The report says that a `blob:` URL iframe gets a unique opaque origin regardless of the `allow-same-origin` flag. That is wrong. A `blob:` URL that the host creates keeps the origin of its creator when the frame has `allow-same-origin`. The origin is opaque only without `allow-same-origin`, and that conflicts with the proxy relay (`apps.mdx:475`). Correction: `blob:` is not an origin-separation fix. The candidate that gives a real distinct origin is a custom protocol with one origin for each server, app and version. See `critique-engineering.md` section 2. `TASK_2026_497_debb` rejects `blob:`.
- **E3.** "Verdict", "Fit for Ptah" part (c), and "Work items", app-authoring item 3. The report says that a text fallback is "mandatory by spec". The spec (`apps.mdx:1558-1559`) says that servers SHOULD provide a text-only fallback. It also says that tools MUST return a meaningful content array even when UI is available. Correction: the text fallback for every UI-linked tool is a Ptah product requirement. It is stricter than the spec.
