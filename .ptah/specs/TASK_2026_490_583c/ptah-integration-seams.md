...
| `message_complete`                                                                                                                   | `TEXT_MESSAGE_END` + `RUN_FINISHED`    | cost, tokens, model need `CUSTOM` |
| `thinking_start` / `thinking_delta`                                                                                                  | `REASONING_*`                          | `signature` has no field          |
| `tool_start` / `tool_delta` / `tool_result`                                                                                          | `TOOL_CALL_START` / `ARGS` / `RESULT`  | no separate `TOOL_CALL_END`       |
| `turn_state`                                                                                                                         | `STATE_SNAPSHOT`                       | best match, revision-stamped      |
| `agent_status`                                                                                                                       | `STATE_DELTA`                          | a hand-made patch                 |
| `compaction_complete`                                                                                                                | `MESSAGES_SNAPSHOT` (approximate)      | carries no messages               |
