...
- **Rule 3: Optimistic UI with Graceful Fallback**. Every interactive button must immediately reflect an optimistic loading state. If the tool call fails or times out (exceeding 3,000ms), the UI must revert state and surface an inline retry badge (see Errata E2).
...
- **E1.** Section 4, the launch-apps table, column "Why Local Desktop Beats Cloud SaaS", rows 1, 3 and 5. The table says that data "never leave the local machine", that data stay "completely private on-device", and that local scraping "evades cloud bot-blockers". These claims are wrong. AWS, GitHub, Slack, calendar, mail and web scraping all cross external service boundaries. Requests, results and alerts reach those services. Correction: the data is not proxied through a Ptah-managed cloud. Credentials stay on the device. Scraping originates from the IP address of the user. The report makes no claim about bot blockers.
- **E2.** Section 5, Rule 3, the revert after 3,000 ms. A timeout means that no response arrived. It does not mean that the operation failed. A write can still commit after the timeout. A revert followed by a retry can duplicate the mutation or hide a completed destructive action. Correction: each mutation that an app starts carries an operation ID (an idempotency key). On timeout the UI shows a pending or unknown state and queries the result again by operation ID. The UI rolls back only when cancellation or compensation is guaranteed.
