...
description: >-
  ...
  branch, returning `cfg.defaultTier`, and its own docblock states the reason: a
  pinned dated Claude id reaches a non-Anthropic endpoint verbatim and 404s. The
  inherit branch does not. Since every lane ships `provider: ''`, inherit is the
  DEFAULT path, so a user who moved their main provider to Z.AI, Moonshot or
  ...
  because the current behaviour is deliberate and load-bearing, not an oversight
  — see context.
updated: '2026-08-25T21:11:52.910Z'
---
