...
Rebase 3 (2026-09-25, after Batch 25): `main` gained 32 commits (PRs #588, #596). The branch was rebased onto `origin/main` `9afac1aa2` with no conflict (`tsconfig.base.json` auto-merged; backup `task533-backup-pre-rebase3` = `da2908482`). After the rebase: `lint,typecheck,test` green for the 13 touched projects (`--parallel=2`); Marketplace e2e 30/30 (28 Batch 25 + 2 Batch 25a) on a fresh production build; R7 re-probe 0/109 `BRAND_MARKS`-only paths in the 13 initial chunks, initial JS 3,161,767 B (+1,611 B vs the Task 25.3 figure, all from the 32 `main` commits — the 533 content is unchanged). Commit hashes in the Batch 25/25a/25b records are PRE-rebase-3.
...
## PR #595 CI fixes (2026-09-25, after the PR was marked ready) — COMPLETE

- The user reviewed PR #595: 268 files (CodeRabbit skips > 100), GitGuardian failed (3 "Generic High Entropy Secret" hits in test fixtures), SonarCloud failed (Security D, Reliability E on new code; 26 issues), and CodeRabbit's `!**/*.generated.*` filter hid the two brand tables. Decisions (user): keep ONE PR (a split needs 3 stacked PRs to stay ≤ 100 files each); rename the brand tables to `*.vendored.ts`; clear GitGuardian by fixup + force-push.
- GitGuardian: the fake secrets in `provider-row.spec.ts` (Batch 8), `server-detail.component.spec.ts` (Batch 13) and, proactively, `marketplace-inventory.store.spec.ts` (Batch 5) are now low-entropy values (`env-value-must-not-render` etc.). Folded into the original commits with `--fixup` + `rebase --autosquash` (backup `task533-backup-pre-ggfix`); `git log -S` finds none of the old values in the branch history. 95 + 54 tests green.
- F1 (backend-developer `ci-f1`, worktree `task533-ci1`): `brand-marks.generated.ts` → `brand-marks.vendored.ts`, `provider-brand-art.generated.ts` → `provider-brand-art.vendored.ts`; manifest, script path validators, imports, comments and the four eager-import guard specs updated (guards shown to still fail on an eager import). Sonar fixes in `scripts/brand-icons/*`: S2189 (false positive — loop restructured so termination is visible), S2871 (one shared code-unit `compareStrings` in `path-geometry.mjs`), S8786 ×2, S7773 ×13, S7781 ×2. 800k-case fuzz vs the old code: 0 differences; `vendor:brand-icons --check` passes. Reviews: logic APPROVED 9/10 (own fuzz), style NEEDS_REVISION 7/10 → APPROVED 9/10 (stale comments, duplicate comparator, regex comments).
- F2 (frontend-developer `ci-f2`, worktree `task533-ci2`): S1523 — `marketplace.fixtures.ts` resolves dynamic fixtures on the Node side through one `page.exposeFunction` binding instead of `new Function` in the page; S2871 + S8786 in `provider-row.ts` (code-unit comparator, linear `parseAssignment`); S8786 ×3 in `libs/shared` `mcp-server-identity.ts` (linear `trimEdgeRuns`; only marketplace and ui call it); S7781 `skill-detail.component.ts`; S7758 `monogram-tile.component.ts` (code-point hash — tints unchanged without astral characters). 300k-case differential fuzz: 0 differences (reviewer re-ran 800k+). Reviews: logic APPROVED 9/10, style APPROVED 9/10.
- Commits: `355db52f7` refactor(scripts,ui,chat-ui), `168841dc6` fix(marketplace,shared,ui,e2e). Checks in TASK_WT: `lint,typecheck,test` green for the 13 touched projects; Marketplace e2e 30/30; R7 0/109 `BRAND_MARKS`-only paths in the 13 initial chunks, initial JS 3,161,868 B (+101 B: code-point hash and trim helper).
- Follow-ups: `scripts/vendor-brand-icons.mjs:131` keeps its own `compareStrings` (pre-existing); the "install the auto-responder once per page" rule in `marketplace.fixtures.ts` is documented, not enforced; no e2e covers a throwing fixture resolver; `thoth/skills-lane-pickers.e2e.spec.ts:313` still uses `new Function` (pre-existing, not new code). With the rename, Sonar and CodeRabbit now read the vendored tables; a Sonar size or duplication finding on them would need a Sonar-side exclusion.
