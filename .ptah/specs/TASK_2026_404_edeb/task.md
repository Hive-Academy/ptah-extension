...
Filed as TASK_2026_392, then as TASK_2026_400, and collided with an unrelated
task both times. Each id was allocated as "highest local NNN + 1" inside a
worktree that could not see the other branches, so the number was already taken
on `main` before this branch reached it. The suffix is what makes a future
collision harmless: two folders can carry 404 and stay distinct instead of one
silently overwriting the other on rebase.
