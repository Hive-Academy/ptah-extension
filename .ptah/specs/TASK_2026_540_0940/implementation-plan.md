...
| Center panel renders `<ptah-app-shell class="h-full w-full" />`; the component already runs an `effect()` in its constructor; the template uses no `viewChild` today | `electron-shell.component.ts:264`, `:343` | The keyed remount host wraps `:264`; the focus effect joins the existing constructor effect |
 ...
 * The Electron shell keys its <ptah-app-shell> host on this value, so each
 * bump destroys and re-creates the shell and its routed surface against the
 * new workspace — same URL, new component instance, no navigation.
 ...
 * so the tick stays at 0 there and the key never changes.
 */
...
**What the user sees after a switch while a configuration surface is open.** The surface stays on screen and its component is re-created against the new workspace (Decision 2's keyed remount). The workspace sidebar switches to the new workspace; no tab in the row is active; the menu trigger keeps its active indication. Config-surface BACK buttons still go to chat (`settings.component.ts:182`, `thoth-shell.component.ts:273`, marketplace hub `:159` — finding 7), and the new workspace's remembered surface is served only when a later switch lands on B through the restore at `:714-716` (finding 7). Tab clicks navigate to the clicked surface through the ordinary path.
...
### Decision 2 — Pre-workspace render path, the keyed remount, and focus
...
**2. First folder opens while a configuration surface is shown.** The gate flips from branch 2 to branch 3: the bare outlet is destroyed, and `ptah-app-shell` (inside the keyed host below) is created. The destroyed outlet's stored route survives (`_router-chunk.mjs:1277-1283`), and the new outlet inside `ptah-app-shell` re-activates it — same URL, fresh component (`:1747-1763`). If `switchWorkspace` runs for that first folder and the surface is a configuration surface, the stay-branch bumps the remount tick; the bump is harmless — the branch flip already re-created the area, and the keyed host's first render uses the current tick.
...
**5. Keyed remount host (user decision 2).** Wrap the center-panel `ptah-app-shell` (`:264`):

```
<div class="h-full w-full min-w-0 flex-1" tabindex="-1" #configurationSurfaceHost>
  @for (k of [appState.configurationSurfaceRemountTick()]; track k) {
    <ptah-app-shell class="h-full w-full" />
  }
</div>
```

(The implementer keeps the center panel's existing classes and adds `tabindex="-1"` plus the template ref.) On every tick change the `@for` block destroys and re-creates `AppShellComponent`. Destroying it destroys its `RouterOutlet`; the Router keeps the stored route (`_router-chunk.mjs:1738-1743` → `:1277-1283`), and the new outlet's `ngOnInit` re-activates it via `activateWith` (`:1747-1763`) — **a new component instance at the same URL, with no navigation**. No RouteReuseStrategy, no `[class.hidden]`, no `retain: true`.

Why the whole shell and not just the routed component: the outlet lives inside `app-shell.component.html:47-49`, a shared file this task must not touch (user decision 1), and keying the host achieves the same fresh component through Angular's own keyed-block semantics.

- Marketplace child routes (TASK_2026_533): the destroyed outlet is the top-level primary outlet; its stored route is the marketplace branch. `activateWith` re-creates the whole activated tree, child outlets included. The URL and Router state are untouched, so the remount lands on the same child-route URL.
- The remount also destroys the canvas tiles. That is correct here: `OrchestraCanvasComponent` hydrates against `activeWorkspacePath` in its constructor (`orchestra-canvas.component.ts:334-338`), so the re-created canvas hydrates against the NEW workspace — which is what user decision 2 asks for.
...
**6. Focus after the remount.** Per "Focus ownership" (`design-spec.md:63-76`): after the keyed re-creation, the shell moves focus to the surface's host element, rendered with `tabindex="-1"` so it takes focus without entering the tab order; the surface owns its inner focus. In `electron-shell.component.ts`, alongside the existing constructor effect (`:343`):

```ts
// Focus ownership (TASK_2026_492 design-spec:63-76): after the keyed
// re-creation, move focus to the shell host — out of the tab order, so the
// re-created surface owns its inner focus.
effect(() => {
  const tick = this.appState.configurationSurfaceRemountTick();
  if (tick === 0) return;
  untracked(() => {
    // After the change-detection flush that re-creates the host.
    queueMicrotask(() =>
      this.configurationSurfaceHost()?.nativeElement.focus(),
    );
  });
});
```

with `configurationSurfaceHost` a `viewChild` signal for the template ref above (the file's first `viewChild`; `effect` is already imported at `:23`).

**Rejected alternatives for the remount mechanism** (user decision 2 required an evaluation of at least three):
...
- Purpose: mount the menu; apply the code-workspace tab set; the three-branch gate with the bare pre-workspace outlet; the keyed remount host and focus effect; back-to-welcome.
- Responsibilities: remove the four configuration tab buttons (`:146-156, 179-211`) and handlers `openThoth/openSetupHub/openMarketplace/openSettings` (`:386-407`); relabel Canvas→Chat (title "Chat") and Dashboard→Analytics (title "Analytics"); order Chat, `<!-- Apps tab (TASK_2026_494) renders between Chat and Tasks -->`, Tasks, Tribunal, Analytics; keep `onCanvasTab` (`:381-384`), `openDashboard`, `openTribunal`, `openTasks` unchanged (criteria 11, 12); keep `role="tablist"`/`role="tab"`/`aria-selected`, the `electron-tabs` class and the `:122` gate (criterion 13; `activity-ticker.e2e.spec.ts:126` selects `[role="tablist"].electron-tabs`); mount `<ptah-global-config-menu />` in the cluster immediately left of `<ptah-theme-toggle />` (`:223-227`); the three-branch gate, keyed remount host, focus effect and back-to-welcome button per Decision 2; delete the icon fields that become unused (`WrenchIcon`, `StoreIcon`, `RadioTowerIcon` after the tabs move into the menu — they move to the menu component; the already-unused `ZapIcon`, `BotIcon`, `GitBranchIcon`, `SparklesIcon` at `:368-371` are deleted as dead code; `SettingsIcon`/`BarChart3Icon` stay on the menu component's items / the Analytics tab respectively).
...
- Failure behaviour: pre-workspace rendering of a config surface cannot fail closed — if a lazy chunk fails, `openSurface` stays null and the welcome screen remains, which is the honest fallback. A remount cannot "fail": the keyed block is pure template semantics; the stored route always exists because the surface was on screen when the switch landed.
...
4. Readers: menu trigger highlight and item `aria-current` (`openConfigurationSurface`); the three-branch gate and back-to-welcome visibility (Decision 2); the keyed remount host (`configurationSurfaceRemountTick`).
5. Workspace switch while a configuration surface is open: stay-branch (Decision 1) → tick bump → `@for` re-creates `AppShellComponent` → its outlet re-activates the stored route (new component instance, same URL) → focus effect moves focus to the host.
...
- **Workspace switch while a configuration surface is open**: stay-branch — no navigation to fail; `_settlementOwner` transferred synchronously; the keyed remount re-creates the surface. If the remounted surface still shows stale data for the new workspace, that indicates a service-level cache in that pane's lib (see Risks, A2) — the remount itself cannot fail.
...
- **TASK_2026_524** (batch 3, RouteReuseStrategy owner): 540 adds no reuse strategy. The remount deliberately avoids your slot: it works through keyed block destruction plus the Router's own outlet re-activation. Your batch stays free to land its strategy without conflicts from this task.
...
| 21 | No new `[class.hidden]` or `retain: true`; keyed `@for` remount instead; added-lines grep guard in Test plan |
