...
description: >-
  ...
  created for CLI/TUI/gateway/cron hosts) and a copy engine for rival CLIs
  whose cleanup is dead code and whose hash cache is never consulted. The SDK
  `plugins:` option is threaded through four services and dropped. Sessions
  can start before junctions exist, headless sessions never get them, promoted
  ...
  `~/.ptah/user`, per-target adapters, hash-gated idempotent copies, no
  removal on deactivate, reconcile at every entry point plus a session-start
  preflight, and a `HarnessHealth` report surfaced as `ptah harness doctor`
  and in the UI.
---
