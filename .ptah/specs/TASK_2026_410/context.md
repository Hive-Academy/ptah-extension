...
Status: PARTIAL. Only the unblockers shipped. The core scope has not started.

Shipped:

- `0caa52f27` — portable worktree paths (`worktree-hook-handler.ts`, NUL-delimited `git worktree list -z` parsing) and `strict: false` in `responses-request-translator.ts`.

Remaining targets:

- [ ] Live execution view for a background agent (text, tool calls, results).
- [ ] Tray chip click opens or focuses that view. Today `onFocus` in `background-agent-tray.component.ts` only calls `tabManager.switchTab`.
- [ ] Identity reconciliation between toolCallId and the real SDK agentId.
- [ ] Keep streaming text and tools after the parent turn completes.
- [ ] Bounded retention and historical backfill for background runs.
