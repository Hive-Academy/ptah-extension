...
description: >-
  ...
  never provides `MODEL_REFRESH_CONTROL`, which
  `HarnessWorkflowMessageHandler → HarnessWorkflowService →
  PermissionHandlerService → TabManagerService` now injects, so all six tests
  die with NG0201. `libs/frontend/thoth-shell` fails on
  `this.state.refreshQueue is not a function` with
