...
description: >-
  ...
  /ptah-core:orchestrate" and burns 0 tokens; the prompt must be `/orchestrate
  <TASK_ID>`. (2) Start is gated only on terminality — the card renders it for
  every status except `done`/`cancelled` (`task-card.component.ts:400`,
  `isTerminal()` at `:670-671`) and the row does the same via `group.terminal`
  (`task-list.component.ts:578,623,806`). It must render only for `backlog` and
  `blocked`; `in_progress` and `in_review` already have a run and should not
  offer a second launch. Reported 2026-08-16 from the board UI.
updated: '2026-08-25T21:21:44.947Z'
---
