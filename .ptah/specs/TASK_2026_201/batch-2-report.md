...
landed**. Batch 3 must _verify and keep_ it, not re-add it, and Task 3.2 still owes it spec
...
`durationPreset`. But §1 also requires the audit metadata be _"byte-for-byte the metadata at
`:536-548`"_, and that metadata contains `durationPreset: dto.durationPreset`. The two
...
> caught _inside_ an interactive transaction cannot be retried inside it — the retry would
> issue its next statement into an aborted session while still _looking_ like a retry.
...
Swapped, and the comment at `:590-593` rewritten: _"A gift is not a conversion. This used to
call `markConverted`, which polluted the paid-conversion funnel with every free grant;
`convertedAt` is now written by exactly one thing, the Paddle provisioning fan-out."_
...
- the 409 raised _inside_ the transaction is **not** retried (one `$transaction` call);
...
1. **`WaitlistService.markApproved` has no spec yet.** Task 3.2 owes it three cases (stamps ·
   ...
   `license.service.spec.ts` covers the _call site_ (`markApproved` invoked once with the
   lowercased email; `markConverted` never invoked); the _method body_ is untested until 3.2.
...
generated _inside_ the core, so each re-entry gets a fresh key. The spec proves it behaviourally:
...
comes from _deleting_ `inviteBatch`, correctly left to Batch 3. Rejecting would produce a commit
