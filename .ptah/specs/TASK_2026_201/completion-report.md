...
- **C2 — the module table was amended**, replacing `task-description.md` §4. The
  ...
  weighting. Consequence worth keeping: the agent (Day 8) now lands *after*
...
auto-discovered. The report's *substance* is correct and I reproduced it; only the
...
3. **Playwright was never executed.** Not in Batch 6, not in Batch 7. The new
   `admin-waitlist-approve.spec.ts` has only ever been *collected*
...
approve is the *free* path and is not conditional on the checkout flip at all, so
...
The failure mode this is shaped against is *a mis-typed start date silently
shifting ten member-visible dates*. A `confirm: true` boolean was rejected —
