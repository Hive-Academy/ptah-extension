...
| `admin_audit_log` rows with `action='license.complimentary.issue'` (bonus — proves the *whole* row rolled back, not just the outer half) | ✅ **0**, despite the service's own log line ("admin audit log recorded … action: license.complimentary.issue") firing during the transaction — the log fires on write, the DB row did not survive the rollback |
