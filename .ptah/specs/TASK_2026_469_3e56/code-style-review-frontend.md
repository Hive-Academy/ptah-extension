...
Consumers outside `git-ui` (such as the Electron shell, e2e specs, or companion libs) needing to access stash state, trigger mutations, or provide mocks will be forced into deep imports because [`GitStashService`](libs/frontend/git-ui/src/lib/services/git-stash.service.ts#L78-L84) was omitted from [`src/index.ts`](libs/frontend/git-ui/src/index.ts#L13-L24). When the module boundaries linter or build configuration tightens to disallow secondary entry points, those deep imports will fail. The original effect-write concern was superseded by fix-round-1 frontend Item 8, which moved confirmation state to `linkedSignal` and left the effect responsible only for loading.

### 2. What would a new team member misread?

A new engineer reading [`diff-tabs.service.ts:321`](libs/frontend/git-ui/src/lib/services/diff-tabs.service.ts#L321) where `toTab()` sets `comparison: 'staged'` on historical diffs would misread that stash diffs are staged working-tree diffs capable of hunk-staging operations, rather than understanding that `'staged'` is merely an artificial value satisfying the strict wire union while `provenance.kind === 'historical'` overrides behavior.

### 3. What does this cost to maintain?

[`DiffTabsService`](libs/frontend/git-ui/src/lib/services/diff-tabs.service.ts#L91) now stands at 883 lines. While this PR responsibly limited its addition to a clean 10-line tab updater without inlining stash logic, creeping additions to this service keep it well above the 700-line soft ceiling. Without an intentional facade extraction (`DiffHunkApplierService` or `DiffTabRevalidatorService`), future git features will continue to incrementally grow the file toward the 1,000-line hard ceiling.

### 4. Where is this inconsistent with the rest of the repository?

1. [`GitStashService`](libs/frontend/git-ui/src/lib/services/git-stash.service.ts#L78) is the only state service in `src/lib/services/` not exported from [`src/index.ts`](libs/frontend/git-ui/src/index.ts#L13-L24) (unlike `GitStatusService`, `GitBranchesService`, `WorktreeService`, `SourceControlService`, `DiffTabsService`, `EditorLauncherService`, and `GitReviewService`).
2. Imperative reset `this.confirmDrop.set(null)` inside an effect in [`stash-popover.component.ts:188-191`](libs/frontend/git-ui/src/lib/stash/stash-popover.component.ts#L188-L191) deviates from modern Angular 21 `linkedSignal` usage for resetting local component state on input changes.
3. In [`open-in-button.component.ts:67`](libs/frontend/git-ui/src/lib/open-in/open-in-button.component.ts#L67), `@if (available().length > 0)` renders a dropdown caret even when exactly 1 target exists, creating a redundant single-item dropdown menu.
...
- File: [`libs/frontend/git-ui/src/index.ts:13-24`](libs/frontend/git-ui/src/index.ts#L13-L24)
...
- Recommendation: Export `GitStashService` and `type GitStashMutation` in [`libs/frontend/git-ui/src/index.ts`](libs/frontend/git-ui/src/index.ts).

### 2. Signal write inside `effect()` instead of `linkedSignal`

- File: [`libs/frontend/git-ui/src/lib/stash/stash-popover.component.ts:188-191`](libs/frontend/git-ui/src/lib/stash/stash-popover.component.ts#L188-L191)
...
- Status: **Superseded by fix-round-1 frontend Item 8.** The finding is retained as historical review evidence, but it is no longer active.
...
- File: [`libs/frontend/git-ui/src/lib/stash/stash-popover.component.ts:99-143`](libs/frontend/git-ui/src/lib/stash/stash-popover.component.ts#L99-L143)
...
- File: [`libs/frontend/git-ui/src/lib/git-dock/git-dock-header.component.ts:164,181`](libs/frontend/git-ui/src/lib/git-dock/git-dock-header.component.ts#L164-L181)
...
- File: [`libs/frontend/git-ui/src/lib/open-in/open-in-button.component.ts:199-209`](libs/frontend/git-ui/src/lib/open-in/open-in-button.component.ts#L199-L209)
...
- File: [`libs/frontend/git-ui/src/lib/services/diff-tabs.service.ts:299-309`](libs/frontend/git-ui/src/lib/services/diff-tabs.service.ts#L299-L309)
...
### 5. Single available target still renders a redundant dropdown caret

- File: `libs/frontend/git-ui/src/lib/open-in/open-in-button.component.ts:67`
- Problem: `@if (available().length > 0)` renders a dropdown caret when there is exactly one target, even though the primary action already opens that only target.
- Impact: Adds a redundant one-item menu and unnecessary keyboard stop.
- Fix: Render the caret only when more than one target is available. The owner decision recorded below intentionally leaves this finding open.
...
Score 8/10 — [0 B, 0 S, 2 M]. Clean signal architecture and OnPush change detection. Properly filters out `terminal` when a file path is bound. Good keyboard handling for `Escape` and outside clicks. Minor issues: redundant caret when `available().length === 1` ([line 67](libs/frontend/git-ui/src/lib/open-in/open-in-button.component.ts#L67)), and missing focus restoration when choosing a target via keyboard ([line 199](libs/frontend/git-ui/src/lib/open-in/open-in-button.component.ts#L199)).
...
Score 7/10 — [0 B, 1 S, 1 M]. Implements OnPush change detection, inline confirmation for destructive `Drop` operations, status-labeled file rows, and outside-click handling. Serious finding: signal write in `effect()` for `confirmDrop.set(null)` ([lines 188-191](libs/frontend/git-ui/src/lib/stash/stash-popover.component.ts#L188-L191)) instead of `linkedSignal`. Minor finding: missing contextual `aria-label` on repeated action buttons ([lines 99-143](libs/frontend/git-ui/src/lib/stash/stash-popover.component.ts#L99-L143)).
...
| ----------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ChangeDetectionStrategy.OnPush` is mandatory               | PASS   | [`editor-brand-icon.component.ts:16`](libs/frontend/git-ui/src/lib/open-in/editor-brand-icon.component.ts#L16), [`open-in-button.component.ts:34`](libs/frontend/git-ui/src/lib/open-in/open-in-button.component.ts#L34), [`git-dock-header.component.ts:48`](libs/frontend/git-ui/src/lib/git-dock/git-dock-header.component.ts#L48), [`stash-popover.component.ts:48`](libs/frontend/git-ui/src/lib/stash/stash-popover.component.ts#L48) |
...
| Workspace partitioning for git state                        | PASS   | [`git-stash.service.ts:85-93`](libs/frontend/git-ui/src/lib/services/git-stash.service.ts#L85-L93) partitions state map by active workspace                                                                                                                                                                                                                                                                                                 |
| Type safety: `catch (error: unknown)` with narrowing        | PASS   | [`git-branches.service.ts:552`](libs/frontend/git-ui/src/lib/services/git-branches.service.ts#L552), [`git-stash.service.ts:60,129,171`](libs/frontend/git-ui/src/lib/services/git-stash.service.ts#L60)                                                                                                                                                                                                                                    |
| Kebab-case naming, no helpers/utils fragments               | PASS   | All 14 files follow `kebab-case.{service,component,spec}.ts`                                                                                                                                                                                                                                                                                                                                                                                |
| Single entry point (`src/index.ts`)                         | FAIL   | [`src/index.ts:13-24`](libs/frontend/git-ui/src/index.ts#L13-L24) fails to export `GitStashService`                                                                                                                                                                                                                                                                                                                                         |
| Modern signal derivation (`linkedSignal` over effect write) | FAIL   | [`stash-popover.component.ts:188-191`](libs/frontend/git-ui/src/lib/stash/stash-popover.component.ts#L188-L191) mutates `confirmDrop` inside `effect()`                                                                                                                                                                                                                                                                                     |
