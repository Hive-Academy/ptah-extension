...
## Batch 17b: P3 — origin-aware skill re-propagation (FU-17b) — COMPLETE
...
- Commit: `feat(skill-synthesis): defer background skill re-propagation while user clicks re-propagate at once` (SHA in `git log`; this file is part of that commit).
- Executed by: `backend-developer-b17b` (first pass + review fixes; logs `D:\projects\ptah-437-backup\b17b-*.log`, `b17b-fix-*.log`).
- Reviews: `b17b-code-logic-review.md` base NEEDS_REVISION 7/10 (1 serious: a curator pass could serialize N × 10 min; 1 moderate: `acceptSuggestion` never re-propagated) → delta APPROVE HIGH. `b17b-code-style-review.md` NEEDS_REVISION 7/10 — both serious items (harness-sync `CLAUDE.md` silent on the new field; CLI adapter silently ignoring origin) and minor 4 (two-branch `refresh` forwarding) fixed, verified in the logic delta.
- Caller trace (re-done after Batch 18):
  - Click → `userInitiated: true`: `promote`, `promoteBulk`, `enhanceNow`, `runCurator` (manual), `applyProposal`, `revertEnhancement`, `acceptSuggestion` (RPC handlers in `skills-synthesis-rpc.handlers.ts`).
  - Background → no origin: the curator interval pass (`skill-curator.service.ts` enhancement loop); `SkillInvocationTracker` (registered in DI, no production resolver today).
- Design:
  - Port: `SkillRepropagationPort.repropagate(kind, slug, root, origin?: QueryOrigin)`.
  - harness-sync: `HarnessPropagateOptions.userLayerRefreshReason` and `IUserLayerRefresher.refresh(root, reason?)`; undefined means the reason is omitted; one call site (`harness-propagation.service.ts`).
  - Electron (`skill-repropagation.ts`): a click maps to the ungoverned `harness-propagation` reason and is awaited; background maps to the new governed `skill-repropagation` reason (`SKILL_REPROPAGATION_USER_LAYER_REASON`, listed in `GOVERNED_USER_LAYER_REASONS`) and is fire-and-forget — the private `propagate` never rejects and logs one warn on failure. N background calls in one curator pass merge into one held coalescer batch (spec: 3-candidate loop, one `whenClear`, one `mirrorAll`). A click joining a held background batch releases it (the batch runs ungoverned).
  - `SkillCuratorService.acceptSuggestion` is now async and re-propagates the accepted skill (pre-existing gap closed; non-fatal on port failure; empty workspace root skips).
  - CLI `CliSkillRepropagation` accepts and ignores origin (no governor in the CLI). VS Code resolves the base `NoOpSkillRepropagation` (pre-existing limitation).
  - Rejection rule unchanged from Batch 17 (`AbortError` skips, other rejection warns once and runs); `activation` never waits.
- Evidence (executor final run, `b17b-fix-test.log`): `run-many -t test` 5 projects (header 5) — harness-sync 386, skill-synthesis 1,476 (37 skipped), rpc-handlers 2,999 (33 skipped), ptah-electron 631 (7 skipped), cli-engine 181. `typecheck,lint` same 5: 0 errors, warnings only (`b17b-fix-typecheck-lint.log`). `degradation-audit:lint` TOTAL 303 (`b17b-fix-audit.log`).
- Evidence (team-leader re-run before commit, worktree `D:\projects\ptah-437`, no nx reset, jest process count 0): same 5-project `run-many -t test --parallel=1 -- --maxWorkers=2` (header 5) exit 0, identical counts, 11 of 11 tasks from cache (inputs unchanged since the executor fix run) — `b17b-tl-test.log`; `typecheck,lint` same 5 exit 0, 0 errors — `b17b-tl-typecheck-lint.log`; `degradation-audit:lint` TOTAL 303 — `b17b-tl-audit.log`. Prettier checked on every staged file incl. the review markdown. Backup: `batch17b-final-modified.patch`.
- Follow-ups:
  - FU-17b: CLOSED.
  - FU-17b-a: `plugin-activation.ts` 760 lines (over the 700 soft ceiling; was already over before this batch).
  - FU-17b-b: `skill-curator.service.ts` 725 counted lines (max-lines warning), grown by the `acceptSuggestion` fix.
  - FU-17b-c: VS Code accept/promote/enhance do not re-propagate (pre-existing no-op port).

### Task 17b.1: Govern background skill re-propagation, keep clicks immediate — COMPLETE
...
### P3 PHASE GATE PASSED — Phase 3 CLOSED

Run by team-leader on 2026-09-15 in worktree `D:\projects\ptah-437` at `b93357089` (Batch 17b), clean tree, no nx reset, one command at a time. Logs in `D:\projects\ptah-437-backup\`.

| Command                             | Exit | Projects                                  | Result                                    | Log                             |
| ----------------------------------- | ---- | ----------------------------------------- | ----------------------------------------- | ------------------------------- |
| `npm run lint:all`                  | 0    | 73                                        | 0 errors (2,028 warnings, all warn-level) | `p3-gate-lint-all.log`          |
| `npm run typecheck:all`             | 0    | 93                                        | 0 errors (second run, no cache hits)      | `p3-gate-typecheck-all-2.log`   |
| `npx nx build ptah-electron`        | 0    | 1 + 11 dependencies (10 of 12 from cache) | built                                     | `p3-gate-build-electron.log`    |
| `npx nx run degradation-audit:lint` | 0    | 1                                         | TOTAL 303 (baseline)                      | `p3-gate-degradation-audit.log` |

- The first `typecheck:all` run (`p3-gate-typecheck-all.log`) failed on 15 projects (`api-*`, `ptah-license-server`, `ptah-landing-page-e2e`) with 524 errors, all in `libs/api/**` and `apps/ptah-license-server/**` (first: `libs/api/core/src/index.ts:5` TS2307 `./lib/generated-prisma-client/client`). Cause: the worktree lacked the gitignored generated Prisma client, not branch code (the branch touches no `libs/api/**` or license-server file). The orchestrator ran `npx nx run ptah-license-server:prisma:generate` with a process-scoped placeholder `DATABASE_URL` (no `.env` created); the re-run passed.
- Still open outside the gate: D10 (the only open P2 gate) and AC-10 manual boot evidence (Batch 17 outcome).

---

## Batch 19: P4 — O(E+M) finalization + tab persistence quota back-off (C16, C17) — COMPLETE
...
- Commit: `perf(chat-streaming): finalize session history in one pass and back off tab saves after quota errors` (SHA in `git log`; this file is part of that commit).
- Executed by: `frontend-developer-b19` (first pass, docs fix, audit fix; logs `D:\projects\ptah-437-backup\b19-*.log`, `b19-fix-*.log`).
- Reviews: `b19-code-logic-review.md` APPROVED 8/10 (0 serious, 2 moderate → FU-19b, FU-19c); `b19-code-style-review.md` APPROVED 7/10 (serious 2, the missing `CLAUDE.md` entries, fixed in both libs; serious 1 recorded as FU-19a). The logic reviewer re-ran the three new/changed spec files directly: 9, 9 and 19 tests green.
- Design:
  - C16 (`message-finalization.service.ts`): before the message loop, one pass over `events` builds `messageId → first message_start / first message_complete` (`indexMessageBoundaries`) and one pass over the root trees builds `id → first tree` (`indexTreesById`). The loop only does lookups. First match wins in map iteration order, which is what the replaced per-message `find` calls returned. O(M × E) becomes O(E + M).
  - C17 (`tab-persistence.ts` + `tab-manager.service.ts`): a failed write stores one failure record `{ key, failedAt, attempt, tabCount }`. Later saves to the same key skip serialization while `now < failedAt + min(5 s × 2^attempt, 5 min)`. A smaller tab set, or the teardown flush (`flushPendingSave` passes `ignoreBackoff`), bypasses the window. `attempt` rises only under the same key. A success clears the record. One `console.warn` per back-off step; nothing is attempted or logged inside a window. No timers: the next ordinary save trigger retries. A save skipped by the back-off counts as pending, so teardown writes it once per unload. An error thrown before the storage key is known keeps the original single warning and creates no record.
  - Audit fix (part of this batch): the first cut ended the `attemptedKey === null` catch branch with `return;`, a new `[catch-return-sentinel]` site (degradation audit TOTAL 304, chat-state 3 > baseline 2). The catch is now `if (attemptedKey === null) { warn } else { record + warn }` with no `return`; behaviour unchanged. The baseline was not updated.
- Evidence:
  - Executor: chat-state 387 passed, chat-streaming 501 passed + 1 skipped; equivalence oracle (the pre-C16 loop, verbatim) on 4 fixtures, including a seeded ~2,000-event interleaved session; counting proxy ≤ 2 × E event visits for the new loop, > 10 × E for the legacy loop; `typecheck,lint` 0 errors. After the audit fix: chat-state 17 suites, 387 passed; audit TOTAL 303 (`b19-fix-*.log`).
  - Team-leader (worktree `D:\projects\ptah-437`, no nx reset, runner limit respected): before the audit fix, `run-many -t test` header 2, exit 0, no cache hits — chat-state 387 passed, chat-streaming 501 passed + 1 skipped (`b19-tl-test.log`); `typecheck,lint` header 2, exit 0, 0 errors (`b19-tl-typecheck-lint.log`); audit exit 1, TOTAL 304 (`b19-tl-audit.log`). After the fix: `degradation-audit:lint --skip-nx-cache` exit 0, TOTAL 303, chat-state 2 and chat-streaming 2 at baseline (`b19-tl-audit-2.log`); chat-state test 17 suites, 387 passed (`b19-tl-test-2.log`); chat-state `typecheck,lint` 0 errors (`b19-tl-typecheck-lint-2.log`). Prettier checked on every staged file, including the review markdown. Backup: `batch19-final-modified.patch`, `batch19-final-untracked.tar`.
- Crash-window analysis: a renderer crash (no `pagehide` / `beforeunload` / `visibilitychange`) inside a back-off window loses changes since the last successful write, up to the window length. That staleness window predates C17: before this batch, a failed write also left `_lastPersisted` stale, and every later attempt failed again. C17 removes the repeated main-thread `JSON.stringify` inside that window without widening it. A future "suspected freeze" hook could call `flushPendingSave()`.
- Follow-ups:
  - FU-19a: extract a `TabPersistenceCoordinator` (persistence fields, `flushPendingSave`, `_doSaveTabState`, and the `quota back-off` spec block) from `tab-manager.service.ts` (2,628 lines) by the facade rule.
  - FU-19b: one `_persistFailure` per `TabManagerService`, not per workspace key. Two workspaces failing in turn reset `attempt` to 0 and never escalate past 5 s.
  - FU-19c: the "tab set shrank" retry trigger counts tabs only; a smaller payload with the same tab count waits out the window.
  - FU-19d: `extractTextForMessage` is O(U × T) per tile. Negligible at AC-11 scale; revisit with Q6 (history paging).
  - FU-19e: `TabWorkspacePartitionService` background-workspace writes have no back-off.
  - FU-19f: spec tsconfig type errors (`tsc -p tsconfig.spec.json`) pre-exist in other chat-streaming and chat-state spec files (`message-finalization.service.spec.ts`, `tab-manager.lifecycle.spec.ts`, older lines of `tab-manager.persistence.spec.ts`). The `typecheck` target covers only `tsconfig.lib.json`. The new spec files add none.

### Task 19.1: Single-pass indexing in `finalizeSessionHistory` — COMPLETE
...
### Task 19.2: Quota back-off — COMPLETE
...
- Follow-ups:
  ...
  - FU-22d (scheduled by Q6, next after the P4 gate): attribution spike, report only — replicate the CDP profile 2–3 times, a source-mapped flame chart to attribute native DOM calls to their caller, and a run with `@formkit/auto-animate` disabled on the transcript list. Output: the fix class for AC-11.
...
- Still open outside the gate: AC-11 NOT MET (Batch 22 outcome; Q6 DECIDED — FU-22d attribution spike next, report only); D10 (the `publish-electron.yml` matrix on all three OSes); AC-10 manual boot evidence (Batch 17 outcome).
