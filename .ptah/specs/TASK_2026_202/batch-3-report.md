...
It got the module to *load* and then failed differently:
...
> *"the fallback is a hand-rolled `Intl.DateTimeFormat` two-pass offset resolver in the
...
| — | *(`jest.census-probe.config.ts` was temporary; deleted — see §6)* |
...
**Tested** — every one of these asserts *zero* `courseModule.update` calls and *zero*
audit rows:
...
scheduled *into* a fold or gap, and a Berlin test would assert against a date the feature
