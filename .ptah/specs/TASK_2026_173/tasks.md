...
## Batch 8: D2 — Hunk Stage / Revert ✅ COMPLETE — `3d6145863` (2026-08-10) — **ONE NAMED EXIT CRITERION UNMET, CARRIED TO BATCH 9**

> ### ⚠️ COMMITTED WITH A DISCLOSED GAP — read this before reading the ✅ above as "all clear"
>
> **The commit decision was taken by the orchestrator, not by 8C, and not by fatigue.** 8C's verdict
> was **"fit to commit on every data-safety guard, except the dispatch's named exit criterion
> `git:applyHunks` exercised end-to-end in Electron, which no pass met"** — and it explicitly
> declined to rule on whether that gap should hold the commit. The orchestrator took that call:
>
> - Every criterion touching the safety of the user's index and working tree — **AC2–AC7, AC9,
>   AC10, AC12, and both halves of AC6** — is proven against **real git 2.54.0.windows.1** in
>   throwaway repositories. Not mocked git.
> - The unmet criterion is **UI wiring in a live host, not data safety.** Real, but a different
>   class of risk.
...
>   Blocking a commit on an environment the user is actively changing is not sensible.
> - The work was **uncommitted in a tree with other sessions writing to it**, and 8B had already
>   misread a `lint-staged` hide-window as catastrophic loss. **Committing was the safer state.**
>
> **A branch commit is not a release.** The unmet criterion is stated plainly in the commit body of
> `3d6145863` and is **Batch 9 register item 15 — required before D2 is considered done.** Do not
> let this ✅ imply the gate was cleared. It was not.
>
> ### 📌 Material correction to `batch-8a-report.md` §7.2 — the offset guards ARE load-bearing
>
...
> invited the check: _"if you can reach one, that is a finding about the design, not the test."_
>
> **8C reached both, and the design claim was wrong.** The reachable window is not a stale client
> patch — it is a **TOCTOU race internal to a single `applyHunks` call**, between the fresh
> `before = await this.diffFile(...)` re-read and the `--check`/apply that follows a few lines
> later. Milliseconds wide, but real on any filesystem shared with another process (a second Ptah
> window, a background hook, a concurrent `git add`). Nothing in the current design closes it.
>
> - **Guard 2 (pre-write refusal)** — sabotaging the index the instant `--check` ran produced real
>   `git apply --check --verbose` output reporting `Hunk #1 succeeded at 32 (offset 5 lines)`, and
>   the guard refused **before any write**.
> - **Guard 3 (post-write rollback)** — sabotaging after `--check` passed produced the same offset
>   report from the real apply, and the `read-tree` rollback left `git diff --cached` **empty**.
>
> **Both guards are load-bearing, not decorative.** Anyone reading 8A §7.2 in isolation would
> conclude they are dead code and delete them. They are not. `batch-8c-verification.md` §2 carries
> the full reproduction. **8A's disclosure is what made this correction possible** — the invitation
> to check was the right call and it paid off.
>
> ### 📌 `batch-8b-report.md` §1 and §10 item 1 are FALSE — annotated in place
>
> 8B opens by claiming an external `git stash` at 22:08:26 destroyed both its own and 8A's work, and
> instructs the team-leader to `git stash pop`. **No such stash ever existed.** The only entry is an
> unrelated pre-existing marketing stash from another branch; every file both passes wrote was
> present on disk throughout; a forced no-cache run gives **16 suites / 337 tests**. `git stash pop`
> was never run and must not be. The report is **annotated in place** so the claim is not inherited
> as fact — the rest of §2–§9 was checked against the tree and is accurate about its own content.
>
> ### 📌 NFR-1 cross-project floor — CANNOT BE ESTABLISHED, stated rather than dropped
>
> `rpc-handlers` and `ptah-electron` were both affected by concurrent out-of-scope work on this
> branch across all three passes. Neither number is a like-for-like comparison against the 1545
> floor. **Reported as a snapshot, not as a pass**, and deliberately not chased. Post-commit
> re-runs against the committed content: `@ptah-extension/editor` **337/337, 16/16 suites**.
...
### Task 8.1: Extend `GitDiffFileResult` with patch + hunks ✅ COMPLETE
...
### Task 8.2: `git:applyHunks` contract + Zod ✅ COMPLETE
...
### Task 8.3: Operation matrix + patch reassembly ✅ COMPLETE
...
### Task 8.4: Server-side staleness + atomicity + forensics ✅ COMPLETE
...
### Task 8.5: Hunk affordances in the diff view ✅ COMPLETE
...
### Task 8.6: Confirmation modal + post-apply refresh ✅ COMPLETE
...
### Task 8.7: Real-repository byte-identity tests ✅ COMPLETE
...
>
> **The register grew from ELEVEN to SEVENTEEN when Batch 8 landed.** Six items were added by 8A,
> 8B and 8C. One of them — **item 14, the Electron end-to-end verification of `git:applyHunks`** —
> is **not** an ordinary follow-up: it is a **named Batch 8 exit criterion that no pass met**, and
> **D2 is not done until it is run**. It is filed with that status attached, not quietly queued
> alongside the LOW-severity polish items.
>
> **This batch FILES; it does not FIX.** That rule is unchanged and it binds hardest on the items
> that look like one-liners — several carry their exact fix in the table, which is what makes the
> temptation real.
>
> **ID allocation**: scan `.ptah/specs/TASK_*` folder names for the highest `NNN` this year and
> increment. **Never** derive an ID from `registry.md` — it is generated and can be stale. The
> highest folder at dispatch time is **`TASK_2026_202`**, so the first new record is
> **`TASK_2026_203`**. That number moved by **three** during Batch 8 alone (`TASK_2026_200`, `201`
...
>
> The R-7 cut line closed the register at eleven. **Batch 8 reopened it and added six**: items
> **12–17**, raised by 8A, 8B and 8C. They inherit the same bar the first eleven were held to —
> **every item carries a concrete one-line fix, not an open question.**
>
> **Item 14 is not like the others.** It is a **named Batch 8 exit criterion that no pass met**
> (`batch-8-dispatch.md` §7), filed as **REQUIRED before D2 is called done** rather than as a
> discretionary follow-up. Do not let it settle into the middle of a polish list.
>
> **One candidate was considered and deliberately NOT filed**: 8A asked whether the `--check` dry
> run earns its place, having found that removing it fails 0 of 28 tests. **8C ruled on it in §4 and
> the ruling is RETAIN** — `--check` is not load-bearing for final-state correctness (guard 3 is a
> comprehensive backstop) but it eliminates the window in which a real working-tree or index write
> exists in a transiently wrong state, observable by a concurrent editor or by Electron's own
> `.git/index` watcher. **That question is closed, not open. Do not re-file it, and do not let a
> future reviewer delete `--check` as redundant.**
>
...
> behaviour attached, correctly untouched under NFR-9 — recorded here so it is not re-discovered as
> a new finding.
...
| 12  | ⛔ **`git:applyHunks` has NEVER been exercised end-to-end in Electron — a NAMED BATCH 8 EXIT CRITERION THAT NO PASS MET, and D2 IS NOT DONE UNTIL IT IS RUN.** Not discretionary polish; do not file it as though it were. 8A could not run it (no frontend caller existed yet), 8B could not (jsdom only), and 8C attempted it and stopped, judging Electron GUI-driver setup from scratch unaffordable within its budget. Commit `3d6145863` was taken anyway, deliberately, with the gap stated in its body: every data-safety guard is proven against real git, `ptah-electron` was red from concurrent out-of-scope work, and leaving the batch uncommitted in a tree other sessions were writing to was the riskier state. **What is unproven is that a click wires through to the RPC correctly in a running app** — not that the write path corrupts anything. **Fix: add a Playwright `_electron` smoke under `apps/ptah-electron-e2e/src/specs/editor/` that opens a `worktree` diff on a scratch repo, stages one hunk via the keyboard toolbar, and asserts `git diff --cached` contains that hunk and only that hunk.**                                  | `batch-8-dispatch.md` §7 exit list; 8A §7.3, 8B §7.2, 8C §9.1            | **HIGH — REQUIRED BEFORE D2 IS DONE** |
| 13  | **The pre-write offset guard does not restore, so its "Nothing was changed" message is accurate about the tool's own actions but not about the file.** 8C reached the guard under an adversarial concurrent write (the TOCTOU window internal to one `applyHunks` call) and found the guard-2 branch returns directly without calling `restoreAfterFailedApply`, on the reasoning that `--check` is a dry run and nothing needs undoing. That reasoning holds for the service's own writes and **does not** hold for an external write landing in that exact window — which the guard provably can now observe. Guard 3's branch calls `restoreAfterFailedApply` unconditionally and its `read-tree`/bytes-rewrite erases concurrent sabotage as a side effect, so the two branches give different guarantees for the same class of message. **Not a corruption risk**: the tool never writes the user's selected hunk when this fires, and the leftover content is exactly what an external actor wrote. **Fix: call `restoreAfterFailedApply` in the guard-2 branch too, using the `worktreeRestoreBytes` restore point already captured earlier in the function.** | `batch-8c-verification.md` §3                                            | MODERATE                              |
| 14  | **`applyHunks` inherits the service-wide assumption that `workspacePath` is the repository top level, and it is written down nowhere.** Every `GitInfoService` method already assumes it (`readBlob` uses root-relative `rev:path`; `readWorktreeBlob` joins `workspacePath + path`). If a user opens a **subdirectory** of a repo, `git diff` emits root-relative paths while `git apply` resolves them relative to cwd, so the apply **fails safely — it cannot corrupt.** Correctly left alone under NFR-9 as pre-existing and out of scope, but a safe failure nobody has documented reads as a bug to whoever hits it. **Fix: resolve the top level once via `git rev-parse --show-toplevel` and use it as the apply cwd; until that lands, state the assumption explicitly in `GitInfoService`'s class doc.**                                                                                                                                                                                                                                                                                                                                                   | `batch-8a-report.md` §7.5                                                | MODERATE                              |
| 15  | **No in-editor floating hunk action widget — mouse ergonomics only, flagged rather than rounded up.** Task 8.5 asked for glyph-margin decorations **plus an overlay widget** anchored at `modifiedStart`. 8B built the decorations and the roving-tabindex keyboard toolbar but not the floating button cluster, and said so: Monaco's line-anchored primitive is a **content** widget (overlay widgets are fixed-position), an interactive one sits in DOM Angular does not manage, and 8B could not run the app to verify it — on the write path, in the batch whose whole standard is "prove it or disclose it". **Every stated criterion is met without it** (AC1, AC10, AC11, AC14, D3 AC6). What is lost: a mouse user clicks the glyph to select, then acts in the header, rather than acting at the hunk. **Fix: add a Monaco content widget anchored at `modifiedStart`, rendered through an Angular embedded view and disposed with the decoration set — and verify it in the same Electron smoke as item 12.**                                                                                                                                             | `batch-8b-report.md` §4 D-1                                              | LOW                                   |
| 16  | **Nobody has ever SEEN the glyph-margin markers.** Sizing, colours and the `color-mix` fallbacks are asserted only as class names in jsdom; whether a marker is visible and legible in light, dark and high-contrast themes is unverified by anyone. Same root cause as item 12 — no live host was reachable in any pass. **Fix: fold a three-theme glyph render with screenshot capture into the Electron smoke from item 12, so the visual claim is made once by a machine rather than never.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `batch-8b-report.md` §7.4                                                | LOW                                   |
| 17  | **A glyph click landing mid-`await` inside `runApply` is guarded but not tested.** `applyInFlight` guards it structurally and the token binding covers the refresh-then-click ordering, but only that ordering has a test; the click-during-RPC ordering has none. An untested guard is exactly the shape both Batch 7's regression guard and 8A's guard 6 proved can be vacuous. **Fix: add a coordinator test that fires a second `applyHunks` while the first RPC promise is still pending and asserts exactly ONE `git:applyHunks` call reaches the wire.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `batch-8b-report.md` §7.6                                                | LOW                                   |
...
- **Both** R-3 follow-up findings filed (`r3-triage.md` §"Follow-up findings" — the generic `unknown` on directory rows, and the generic `unknown` on submodule paths); **none re-suppressed**
- **All SEVENTEEN Task 9.3 candidates filed as their own records; none silently dropped and none fixed in-task**
- **Register item 12 filed with its `HIGH — REQUIRED BEFORE D2 IS DONE` status intact**, not flattened into the polish list
- Every filed record carries a **concrete one-line fix**, not an open question — the bar the first eleven were held to
- Every ID allocated by a **fresh folder scan of `.ptah/specs/TASK_*`**, never from `registry.md`
...
>
> Batch 8 committed (`3d6145863`) with that named exit criterion **explicitly unmet**, and it is
> filed as **Batch 9 register item 12, HIGH, required before D2 is done**. Every D2 data-safety
> criterion is proven against real git 2.54.0.windows.1; what is unproven is that the UI wires a
> click through to the RPC in a running host. **Do not sign off DoD item 1 for D2 until item 12 is
> discharged.**

1. Every acceptance criterion in A1–A4, B1–B5, C1–C2, D1–D3 passes, **verified in Electron** — ⚠️ **D2's Electron verification is OUTSTANDING, see the block above**
