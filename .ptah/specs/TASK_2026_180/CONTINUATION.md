...
- `D:/projects/ptah-extension` is a **different worktree** on unrelated TUI work. Do not touch it. It also holds `TASK_2026_236`, which this worktree does not — **scan both when allocating a task id**, or you will burn one. **ID COLLISION HAPPENED HERE — read this.** Ids `237` and `238` were allocated on this branch AND independently in `D:/projects/ptah-extension` during the same session, for different tasks. The branch renumbered ITS two to `240`/`241` because the other worktree's were `in_progress` work. Scanning both worktrees ONCE is not enough when both are being written concurrently — re-check immediately before you commit a new carrier. This worktree now tops out at `TASK_2026_241`; next free is `242`.
...
- **`TASK_2026_240`** — the `[value]`-without-`[selected]` select-binding sweep. One confirmed instance left (`json-schema-form.component.ts:74-87`).
