...
```typescript
const { data } = await client.from('items').select('id, name, status').eq('status', 'active').order('created_at', { ascending: false }).limit(10);
```
...
export default async function PostsPage() {
  const { data: posts, error } = await dbClient.from('posts').select('id, title, created_at, author:users(name)').order('created_at', { ascending: false }).limit(10);
...
export function ItemsList() {
  ...
  useEffect(() => {
    async function fetchItems() {
      const { data, error } = await dbClient.from('items').select('id, name, status').eq('status', 'active');
